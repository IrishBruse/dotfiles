...
- **`tools/archview`** — **`archview analyze`** reads **`tsconfig.json`** in **`cwd`** via **`ts-morph`**, writes a single **`.context/architecture/architecture.md`** (YAML frontmatter + expanded sections: summary, external packages, entrypoints, internal edges, per-file catalog) (imports/exports per file). Install: `cd tools/archview && npm install && npm link` (or **`node tools/archview/bin/archview.js analyze`**).
