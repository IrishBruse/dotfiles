...
Files here mirror `~` and are meant to be stowed into the real home directory (`.config`, `.cursor`, `.agents`, VS Code `Code/User`, fish, hypr, etc.). Stow via `stow.fish` at the repo root (macOS and Linux).

### linux/

Linux Mint packages and bootstrap: `apt.md` (generated by `open.ts` from manually installed packages), `flatpaks.csv`, `open.ts` (dconf + agent-config export on workspace open).
