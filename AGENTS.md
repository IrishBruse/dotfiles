...
- **`tools/atlassian`** — uses Atlassian CLI (`acli`) where noted.
  - **`jira`** — Interactive TUI from `home/.agents/skills/jira-tickets/SKILL.md` (optional path); **`jira sync`** refreshes that markdown from Jira via `acli`. **`jira board`** is an alias for the no-subcommand view.
