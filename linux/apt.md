# apt

|package|version|
| --- | --- |
|7zip|23.01+dfsg-11|
|accountsservice|23.13.9-2ubuntu6|
|acl|2.3.2-1build1.1|
|acpid|1:2.0.34-1ubuntu2|
|adduser|3.137ubuntu1|
|adwaita-icon-theme|46.0-1|
|alsa-base|1.0.25+dfsg-0ubuntu7|
|alsa-topology-conf|1.2.5.1-2|
|alsa-ucm-conf|1.2.10-1ubuntu5.11|
|alsa-utils|1.2.9-1ubuntu5|
|amd64-microcode|3.20250311.1ubuntu0.24.04.1|
|anacron|2.3-39ubuntu2|
|android-libaapt|1:14~beta1-2build3|
|android-libandroidfw|1:14~beta1-2build3|
|android-libbacktrace|1:34.0.4-1build3|
|android-libutils|1:34.0.4-1build3|
|android-sdk-build-tools-common|29.0.3+9|
|android-sdk-common|28.0.2+9|
|android-sdk-platform-tools|28.0.2+9|
|apg|2.2.3.dfsg.1-5build3|
|apktool|2.7.0+dfsg-7|
|apparmor|4.0.1really4.0.1-0ubuntu0.24.04.7|
|app-install-data|15.10build1|
|appstream|1.0.2-1build6|
|apt|2.8.3|
|aptdaemon|1.1.1+bzr982-0ubuntu44mint1|
|aptdaemon-data|1.1.1+bzr982-0ubuntu44mint1|
|aptitude|0.8.13-5ubuntu5|
|aptitude-common|0.8.13-5ubuntu5|
|apt-transport-https|2.8.3|
|apt-utils|2.8.3|
|aspell|0.60.8.1-1build1|
|aspell-en|2020.12.07-0-1|
|at-spi2-common|2.52.0-1build1|
|at-spi2-core|2.52.0-1build1|
|attr|1:2.5.2-1build1.1|
|audacity|3.4.2+dfsg-1build4|
|autoconf|2.71-3|
|autoconf-archive|20220903-3|
|automake|1:1.16.5-1.3ubuntu1|
|avahi-autoipd|0.8-13ubuntu6.2|
|avahi-daemon|0.8-13ubuntu6.2|
|avahi-utils|0.8-13ubuntu6.2|
|baobab|46.0-1build1|
|base-files|13ubuntu10mint22.3.0|
|base-passwd|3.6.3build1|
|bash|5.2.21-2ubuntu4|
|bash-completion|1:2.11-8|
|bc|1.07.1-3ubuntu4|
|bind9-dnsutils|1:9.18.39-0ubuntu0.24.04.5|
|bind9-host|1:9.18.39-0ubuntu0.24.04.5|
|bind9-libs|1:9.18.39-0ubuntu0.24.04.5|
|binutils|2.42-4ubuntu2.10|
|binutils-common|2.42-4ubuntu2.10|
|binutils-x86-64-linux-gnu|2.42-4ubuntu2.10|
|bison|2:3.8.2+dfsg-1build2|
|blueman|2.4.4+mint2+xia|
|bluetooth|5.72-0ubuntu5.5|
|bluez|5.72-0ubuntu5.5|
|bluez-cups|5.72-0ubuntu5.5|
|bluez-obexd|5.72-0ubuntu5.5|
|bolt|0.9.7-1|
|bpfcc-tools|0.29.1+ds-1ubuntu7|
|bpftrace|0.20.2-1ubuntu4.3|
|brasero-common|3.12.3-2build4|
|brightnessctl|0.5.1-3.1|
|brltty|6.6-4ubuntu5|
|bruno|3.2.0|
|bsdextrautils|2.39.3-9ubuntu6.5|
|bsdutils|1:2.39.3-9ubuntu6.5|
|btop|1.3.0-1|
|btrfs-progs|6.6.3-1.1build2|
|bubblewrap|0.9.0-1ubuntu0.1|
|build-essential|12.10ubuntu1|
|bulky|4.2|
|busybox-initramfs|1:1.36.1-6ubuntu3.1|
|busybox-static|1:1.36.1-6ubuntu3.1|
|bzip2|1.0.8-5.1build0.1|
|ca-certificates|20240203|
|caddy|2.6.2-6ubuntu0.24.04.3|
|casper|1.498|
|catdoc|1:0.95-5build1|
|cava|0.7.4-1build2|
|ccache|4.9.1-1|
|cifs-utils|2:7.0-2ubuntu0.2|
|cinnamon|6.6.7+zena|
|cinnamon-common|6.6.7+zena|
|cinnamon-control-center|6.6.0+zena|
|cinnamon-control-center-data|6.6.0+zena|
|cinnamon-control-center-dbg|6.6.0+zena|
|cinnamon-desktop-data|6.6.2+zena|
|cinnamon-l10n|6.6.2|
|cinnamon-screensaver|6.6.1+zena|
|cinnamon-session|6.6.3+zena|
|cinnamon-session-common|6.6.3+zena|
|cinnamon-settings-daemon|6.6.4+zena|
|circle-flags-svg|2.7.0+mint1|
|cjs|115.1+zena|
|ckan|1.36.4.26132|
|clang|1:18.0-59~exp2|
|clang-15|1:15.0.7-14build3|
|clang-19|1:19.1.7-1ubuntu2~kisak~n|
|clinfo|3.0.23.01.25-1build1|
|cliphist|0.4.0-2ubuntu0.3|
|cloc|1.98-1|
|cmake|3.28.3-1build7|
|cmake-extras|1.7-2|
|cmatrix|2.0-6|
|code|1.124.0-1781066808|
|colord|1.4.7-1build2|
|colord-data|1.4.7-1build2|
|command-not-found|23.04.0|
|compiz|1:0.9.14.2+22.10.20220822-0ubuntu12|
|console-setup|1.226ubuntu1|
|console-setup-linux|1.226ubuntu1|
|coreutils|9.4-3ubuntu6.2|
|cpdb-backend-cups|2.0~b5-0ubuntu6|
|cpio|2.15+dfsg-1ubuntu2|
|cpp-13|13.3.0-6ubuntu2~24.04.1|
|cpp-13-x86-64-linux-gnu|13.3.0-6ubuntu2~24.04.1|
|cpp|4:13.2.0-7ubuntu1|
|cppcheck|2.13.0-2ubuntu3|
|cpp-x86-64-linux-gnu|4:13.2.0-7ubuntu1|
|cpupower-gui|0.7.2-2.1ubuntu2|
|cracklib-runtime|2.9.6-5.1build2|
|cron|3.0pl1-184ubuntu2|
|cron-daemon-common|3.0pl1-184ubuntu2|
|cryptsetup|2:2.7.0-1ubuntu4.2|
|cryptsetup-bin|2:2.7.0-1ubuntu4.2|
|cryptsetup-initramfs|2:2.7.0-1ubuntu4.2|
|cups|2.4.7-1.2ubuntu7.13|
|cups-browsed|2.0.0-0ubuntu10.3|
|cups-bsd|2.4.7-1.2ubuntu7.13|
|cups-client|2.4.7-1.2ubuntu7.13|
|cups-common|2.4.7-1.2ubuntu7.13|
|cups-core-drivers|2.4.7-1.2ubuntu7.13|
|cups-daemon|2.4.7-1.2ubuntu7.13|
|cups-filters|2.0.0-0ubuntu4.1|
|cups-filters-core-drivers|2.0.0-0ubuntu4.1|
|cups-ipp-utils|2.4.7-1.2ubuntu7.13|
|cups-pk-helper|0.2.6-1ubuntu8|
|cups-ppdc|2.4.7-1.2ubuntu7.13|
|cups-server-common|2.4.7-1.2ubuntu7.13|
|curl|8.5.0-2ubuntu10.9|
|cursor|3.5.33-1779434134|
|dash|0.5.12-6ubuntu5|
|dbus|1.14.10-4ubuntu4.1|
|dbus-bin|1.14.10-4ubuntu4.1|
|dbus-daemon|1.14.10-4ubuntu4.1|
|dbus-session-bus-common|1.14.10-4ubuntu4.1|
|dbus-system-bus-common|1.14.10-4ubuntu4.1|
|dbus-user-session|1.14.10-4ubuntu4.1|
|dbus-x11|1.14.10-4ubuntu4.1|
|dc|1.07.1-3ubuntu4|
|dconf-cli|0.40.0-4ubuntu0.1|
|dconf-editor|45.0.1-1build2|
|dconf-gsettings-backend|0.40.0-4ubuntu0.1|
|dconf-service|0.40.0-4ubuntu0.1|
|dcraw|9.28-5ubuntu1|
|dctrl-tools|2.24-3build3|
|debconf|1.5.86ubuntu1|
|debconf-i18n|1.5.86ubuntu1|
|debhelper|13.14.1ubuntu5|
|debianutils|5.17build1|
|debtree|1.1.2|
|default-jdk-headless|2:1.21-75+exp1|
|desktop-file-utils|0.27-2build1|
|devtoys|2.0.9|
|devtoys.cli|2.0.9|
|dhcpcd-base|1:10.0.6-1ubuntu3.2|
|dialog|1.3-20240101-1|
|dictionaries-common|1.29.7|
|diffutils|1:3.10-1build1|
|dirmngr|2.4.4-2ubuntu17.4|
|discord|1.0.138|
|distro-info|1.7build1|
|distro-info-data|0.60ubuntu0.6|
|dkms|3.0.11-1ubuntu13|
|dmeventd|2:1.02.185-3ubuntu3.2|
|dmidecode|3.5-3ubuntu0.1|
|dmraid|1.0.0.rc16-12ubuntu2|
|dmsetup|2:1.02.185-3ubuntu3.2|
|dmz-cursor-theme|0.4.5ubuntu1|
|dnsmasq-base|2.90-2ubuntu0.3|
|dns-root-data|2024071801~ubuntu0.24.04.1|
|docbook-xml|4.5-12|
|dosfstools|4.2-1.1build1|
|dotnet8|8.0.127-8.0.27-0ubuntu1~24.04.1|
|doxygen|1.9.8+ds-2build5|
|dpkg|1.22.6ubuntu6.6|
|dpkg-dev|1.22.6ubuntu6.6|
|dpkg-repack|1.52|
|dracut-install|060+5-1ubuntu3.3|
|drawing|1.0.2-1build1|
|e2fsprogs|1.47.0-2.4~exp1ubuntu4.1|
|ecryptfs-utils|111-6ubuntu3|
|ed|1.20.1-1|
|efibootmgr|18-1build2|
|eject|2.39.3-9ubuntu6.5|
|emacsen-common|3.0.5|
|enchant-2|2.3.3-2build2|
|engrampa|1.26.2-5|
|entr|5.5-1|
|espeak-ng-data|1.51+dfsg-12build1|
|evolution-data-server|3.52.3-0ubuntu1.2|
|evolution-data-server-common|3.52.3-0ubuntu1.2|
|exfat-fuse|1.4.0-2|
|exfatprogs|1.2.2-1build1|
|exif|0.6.22-3build1|
|eza|0.18.2-1|
|fakeroot|1.33-1|
|fastfetch|2.64.2~noble|
|fdisk|2.39.3-9ubuntu6.5|
|feh|3.10.1-1ubuntu0.1|
|ffmpegthumbnailer|2.2.2+git20240104+dfsg-1build1|
|file|1:5.45-3build1|
|file-roller|43.0+mint1+wilma|
|finalrd|9build1|
|findutils|4.9.0-5build1|
|fingwit|1.0.8+zena|
|firmware-sof-signed|2023.12.1-1ubuntu1.11|
|fish|4.0.0-2~noble|
|flatpak|1.14.6-1ubuntu0.1|
|flex|2.6.4-8.2build1|
|folder-color-switcher|1.7.1|
|fontconfig|2.15.0-1.1ubuntu2|
|fontconfig-config|2.15.0-1.1ubuntu2|
|fonts-dejavu-core|2.37-8|
|fonts-dejavu-mono|2.37-8|
|fonts-droid-fallback|1:6.0.1r16-1.1build1|
|fonts-firacode|6.2-2|
|fonts-font-awesome|5.0.10+really4.7.0~dfsg-4.1|
|fonts-liberation|1:2.1.5-3|
|fonts-liberation2|1:2.1.5-3|
|fonts-mathjax|2.7.9+dfsg-1|
|fonts-noto-cjk|1:20230817+repack1-3|
|fonts-noto-color-emoji|2.047-0ubuntu0.24.04.1|
|fonts-noto-core|20201225-2|
|fonts-noto-mono|20201225-2|
|fonts-opensymbol|4:102.12+LibO24.2.7-0ubuntu0.24.04.5|
|fonts-ubuntu|0.869+git20240321-0ubuntu1|
|fonts-urw-base35|20200910-8|
|foomatic-db-compressed-ppds|20230202-1|
|foot|1.16.2-2ubuntu0.1|
|fprintd|1.94.3-1|
|freeglut3-dev|3.4.0-1build1|
|friendly-recovery|0.2.42|
|ftp|20230507-2build3|
|fuse3|3.14.0-5build1|
|fwupd|1.9.34-0ubuntu1~24.04.1|
|fwupd-signed|1.52+1.4-1|
|g++-13-x86-64-linux-gnu|13.3.0-6ubuntu2~24.04.1|
|g++-14|14.3.0-12ubuntu1~24~ppa1|
|gamemode|1.8.1-2build1|
|gamemode-daemon|1.8.1-2build1|
|gcc-13-base|13.3.0-6ubuntu2~24.04.1|
|gcc-13-x86-64-linux-gnu|13.3.0-6ubuntu2~24.04.1|
|gcc-14|14.3.0-12ubuntu1~24~ppa1|
|gcc-14-base|14.3.0-12ubuntu1~24~ppa1|
|gcc-14-x86-64-linux-gnu|14.3.0-12ubuntu1~24~ppa1|
|gcc-15-base|15.2.0-14ubuntu1~24~ppa1|
|gcc-16-base|16-20260315-1ubuntu1~24~ppa1|
|gcc-16-base|16-20260315-1ubuntu1~24~ppa1|
|gcc|4:13.2.0-7ubuntu1|
|gcc-x86-64-linux-gnu|4:13.2.0-7ubuntu1|
|gcr|3.41.2-1build3|
|gcr4|4.2.0-5|
|gdb|15.1-1ubuntu1~24.04.1|
|gdisk|1.0.10-1build1|
|genisoimage|9:1.1.11-3.5|
|geocode-glib-common|3.26.3-6build3|
|geoip-database|20240403-1ubuntu1|
|gettext|0.21-14ubuntu2|
|gettext-base|0.21-14ubuntu2|
|gh|2.45.0-1ubuntu0.3|
|ghostscript|10.02.1~dfsg1-0ubuntu7.8|
|ghostty|1.3.0-0~ppa1|
|gimp|3.2.4-1build1~ubuntu2404|
|gimp-help-common|2.10.34-2|
|gimp-help-en|2.10.34-2|
|gir1.2-accountsservice-1.0|23.13.9-2ubuntu6|
|gir1.2-appstream-1.0|1.0.2-1build6|
|gir1.2-atk-1.0|2.52.0-1build1|
|gir1.2-atspi-2.0|2.52.0-1build1|
|gir1.2-ayatanaappindicator3-0.1|0.5.93-1build3|
|gir1.2-camel-1.2|3.52.3-0ubuntu1.2|
|gir1.2-caribou-1.0|0.4.21-9build1|
|gir1.2-cinnamondesktop-3.0|6.6.2+zena|
|gir1.2-clutter-1.0|1.26.4+git2779b932+dfsg-7build3|
|gir1.2-clutter-gst-3.0|3.0.27-4build1|
|gir1.2-cmenu-3.0|6.6.0+zena|
|gir1.2-cogl-1.0|1.22.8-4ubuntu4|
|gir1.2-coglpango-1.0|1.22.8-4ubuntu4|
|gir1.2-cvc-1.0|6.6.2+zena|
|gir1.2-dbusmenu-glib-0.4|18.10.20180917~bzr492+repack1-3.1ubuntu5|
|gir1.2-dee-1.0|1.2.7+17.10.20170616-7build5|
|gir1.2-ecal-2.0|3.52.3-0ubuntu1.2|
|gir1.2-edataserver-1.2|3.52.3-0ubuntu1.2|
|gir1.2-flatpak-1.0|1.14.6-1ubuntu0.1|
|gir1.2-freedesktop|1.80.1-1|
|gir1.2-gdesktopenums-3.0|46.1-0ubuntu1|
|gir1.2-gdkpixbuf-2.0|2.42.10+dfsg-3ubuntu3.3|
|gir1.2-gexiv2-0.10|0.14.2-2build3|
|gir1.2-girepository-2.0|1.80.1-1|
|gir1.2-gkbd-3.0|3.28.1-1build3|
|gir1.2-glib-2.0|2.80.0-6ubuntu3.8|
|gir1.2-gnomedesktop-3.0|44.0-5build2|
|gir1.2-graphene-1.0|1.10.8-3build2|
|gir1.2-gsound-1.0|1.0.3-3.2build2|
|gir1.2-gspell-1|1.12.2-1build4|
|gir1.2-gst-plugins-base-1.0|1.24.2-1ubuntu0.4|
|gir1.2-gstreamer-1.0|1.24.2-1ubuntu0.1|
|gir1.2-gtk-3.0|3.24.41-4ubuntu1.3|
|gir1.2-gtkclutter-1.0|1.8.4-4build5|
|gir1.2-gtksource-3.0|3.24.11-2build4|
|gir1.2-gtksource-4|4.8.4-5build4|
|gir1.2-gtop-2.0|2.41.3-1ubuntu0.24.04.1|
|gir1.2-gudev-1.0|1:238-5ubuntu1|
|gir1.2-handy-1|1.8.3-1build2|
|gir1.2-harfbuzz-0.0|8.3.0-2build2|
|gir1.2-ibus-1.0|1.5.29-2|
|gir1.2-ical-3.0|3.0.17-1.1build3|
|gir1.2-javascriptcoregtk-4.1|2.52.3-0ubuntu0.24.04.1|
|gir1.2-json-1.0|1.8.0-2build2|
|gir1.2-keybinder-3.0|0.3.2-1.1build2|
|gir1.2-matedesktop-2.0|1.26.2-1.1mint1+zara|
|gir1.2-matemenu-2.0|1.26.1+mint1|
|gir1.2-matepanelapplet-4.0|1.27.1-2build3|
|gir1.2-meta-muffin-0.0|6.6.3+zena|
|gir1.2-nemo-3.0|6.6.3+zena|
|gir1.2-nm-1.0|1.46.0-1ubuntu2.7|
|gir1.2-nma-1.0|1.10.6+mint1+wilma|
|gir1.2-notify-0.7|0.8.3-1build2|
|gir1.2-packagekitglib-1.0|1.2.8+mint3+wilma|
|gir1.2-pango-1.0|1.52.1+ds-1build1|
|gir1.2-peas-1.0|1.36.0-3build4|
|gir1.2-polkit-1.0|124-2ubuntu1.24.04.3|
|gir1.2-rb-3.0|3.4.7-2ubuntu6|
|gir1.2-secret-1|0.21.4-1build3|
|gir1.2-soup-2.4|2.74.3-6ubuntu1.6|
|gir1.2-soup-3.0|3.4.4-5ubuntu0.7|
|gir1.2-timezonemap-1.0|0.4.6-6ubuntu1|
|gir1.2-udisks-2.0|2.10.1-6ubuntu1.3|
|gir1.2-upowerglib-1.0|1.90.3-1|
|gir1.2-vte-2.91|0.76.0-1ubuntu0.1|
|gir1.2-webkit2-4.1|2.52.3-0ubuntu0.24.04.1|
|gir1.2-wnck-3.0|43.0-3build4|
|gir1.2-xapp-1.0|3.2.2+zena|
|gir1.2-xkl-1.0|5.4-5build2|
|gir1.2-xmlb-2.0|0.3.18-1|
|gir1.2-xreader|4.6.5+wilma|
|gir1.2-xviewer-3.0|3.4.16+zena|
|git|1:2.43.0-1ubuntu7.3|
|git-delta|0.16.5-5|
|gkbd-capplet|3.28.1-1build3|
|glib-networking|2.80.0-1build1|
|glib-networking|2.80.0-1build1|
|glib-networking-common|2.80.0-1build1|
|glib-networking-services|2.80.0-1build1|
|glslang-tools|15.1.0-2~ubuntu0.24.04.1|
|glslc|2023.8-1build1|
|gnome-accessibility-themes|3.28-2ubuntu5|
|gnome-calculator|1:41.1+mint1+wilma|
|gnome-desktop3-data|44.0-5build2|
|gnome-disk-utility|46.0-1ubuntu3|
|gnome-font-viewer|46.0-1build1|
|gnome-icon-theme|3.12.0-5|
|gnome-keyring|46.1-2ubuntu0.2|
|gnome-keyring-pkcs11|46.1-2ubuntu0.2|
|gnome-menus|3.36.0-1.1ubuntu3|
|gnome-online-accounts|3.50.4-0ubuntu2|
|gnome-online-accounts-gtk|3.50.10+zena|
|gnome-pkg-tools|0.22.9ubuntu1|
|gnome-power-manager|43.0-2build2|
|gnome-screenshot|41.0+mint2+wilma|
|gnome-session-bin|46.0-1ubuntu4|
|gnome-session-canberra|0.30-10ubuntu10|
|gnome-settings-daemon-common|46.0-1ubuntu1.24.04.1|
|gnome-system-monitor|46.0-1build1|
|gnome-themes-extra|3.28-2ubuntu5|
|gnome-themes-extra-data|3.28-2ubuntu5|
|gnupg2|2.4.4-2ubuntu17.4|
|gnupg|2.4.4-2ubuntu17.4|
|gnupg-utils|2.4.4-2ubuntu17.4|
|golang|2:1.22~2build1|
|google-android-cmdline-tools-13.0-installer|13.0+1710437545-3build2|
|google-chrome-stable|149.0.7827.102-1|
|gpg|2.4.4-2ubuntu17.4|
|gpg-agent|2.4.4-2ubuntu17.4|
|gpgconf|2.4.4-2ubuntu17.4|
|gpgsm|2.4.4-2ubuntu17.4|
|gpgv|2.4.4-2ubuntu17.4|
|gpick|0.2.6-1ubuntu2|
|gradle|4.4.1-20|
|graphviz|2.42.2-9ubuntu0.1|
|grep|3.11-4build1|
|grim|1.4.0+ds-2build2|
|groff-base|1.23.0-3build2|
|grub2-theme-mint-2k|1.2.3|
|grub-customizer|5.2.8-0ubuntu1~ppa1n|
|grub-efi|2.12-1ubuntu7.3|
|gsettings-desktop-schemas|46.1-0ubuntu1|
|gstreamer1.0-alsa|1.24.2-1ubuntu0.4|
|gstreamer1.0-clutter-3.0|3.0.27-4build1|
|gstreamer1.0-gl|1.24.2-1ubuntu0.4|
|gstreamer1.0-gtk3|1.24.2-1ubuntu1.4|
|gstreamer1.0-packagekit|1.2.8+mint3+wilma|
|gstreamer1.0-pipewire|1.2.4-1ppa1|
|gstreamer1.0-plugins-base|1.24.2-1ubuntu0.4|
|gstreamer1.0-plugins-base|1.24.2-1ubuntu0.4|
|gstreamer1.0-plugins-base-apps|1.24.2-1ubuntu0.4|
|gstreamer1.0-plugins-good|1.24.2-1ubuntu1.4|
|gstreamer1.0-plugins-good|1.24.2-1ubuntu1.4|
|gstreamer1.0-tools|1.24.2-1ubuntu0.1|
|gstreamer1.0-x|1.24.2-1ubuntu0.4|
|gstreamer1.0-x|1.24.2-1ubuntu0.4|
|gtk2-engines-murrine|0.98.2-4|
|gtk2-engines-pixbuf|2.24.33-4ubuntu1.1|
|gtk-doc-tools|1.34.0-1|
|gtk-update-icon-cache|3.24.41-4ubuntu1.3|
|gucharmap|1:15.1.3-1build1|
|gufw|24.04.0-2|
|gvfs|1.54.4-0ubuntu1~24.04.2|
|gvfs-backends|1.54.4-0ubuntu1~24.04.2|
|gvfs-common|1.54.4-0ubuntu1~24.04.2|
|gvfs-daemons|1.54.4-0ubuntu1~24.04.2|
|gvfs-fuse|1.54.4-0ubuntu1~24.04.2|
|gvfs-libs|1.54.4-0ubuntu1~24.04.2|
|g++-x86-64-linux-gnu|4:13.2.0-7ubuntu1|
|gzip|1.12-1ubuntu3.1|
|hdparm|9.65+ds-1build1|
|heif-gdk-pixbuf|1.17.6-1ubuntu4.3|
|hfsprogs|540.1.linux3-5build3|
|hicolor-icon-theme|0.17-2|
|hollywood|1.21-1.1|
|hostname|3.23+nmu2ubuntu2|
|hplip|3.23.12+dfsg0-0ubuntu5|
|hplip-data|3.23.12+dfsg0-0ubuntu5|
|html2text|1.3.2a-28|
|humanity-icon-theme|0.6.16|
|hunspell-en-us|1:2020.12.07-2|
|hurl|6.0.0|
|hwdata|0.379-1|
|hyphen-en-us|2.8.8-7build3|
|hypnotix|5.6|
|hypridle|0.1.7-1ppa6|
|hyprland|0.55.3-1ppa1|
|hyprland-qt-support0|0.1.0-1ppa4|
|hyprlock|0.9.5-1ppa2|
|hyprpicker|0.4.7-1ppa1|
|hyprpolkitagent|0.1.3-1ppa2|
|i965-va-driver|2.4.1+dfsg1-1ubuntu0.1|
|i965-va-driver|2.4.1+dfsg1-1ubuntu0.1|
|ibus|1.5.29-2|
|ibus-gtk|1.5.29-2|
|ibus-gtk3|1.5.29-2|
|ibus-gtk4|1.5.29-2|
|id3|1.1.2-3build1|
|ideviceinstaller|1.1.1-1build4|
|ieee-data|20220827.1|
|ifuse|1.1.4~git20181007.3b00243-1ubuntu3|
|iio-sensor-proxy|3.5-1build2|
|im-config|0.57-2|
|inetutils-telnet|2:2.5-3ubuntu4.2|
|info|7.1-3build2|
|init|1.66ubuntu1|
|initramfs-tools|0.142ubuntu25.8|
|initramfs-tools-bin|0.142ubuntu25.8|
|initramfs-tools-core|0.142ubuntu25.8|
|init-system-helpers|1.66ubuntu1|
|inputattach|1:1.8.1-2build1|
|insomnia|10.3.0|
|install-info|7.1-3build2|
|intel-gpu-tools|1.28-1ubuntu2|
|intel-gsc|0.9.5-1~24.04~ppa2|
|intel-media-va-driver-non-free|26.2.0-1~24.04~ppa1|
|intel-media-va-driver-non-free|26.2.0-1~24.04~ppa1|
|intel-metrics-discovery|1.14.183-1~24.04~ppa1|
|intel-metrics-library|1.0.209-1~24.04~ppa1|
|intel-microcode|3.20260210.0ubuntu0.24.04.1|
|intel-ocloc|26.18.38308.1-1~24.04~ppa1|
|intel-oneapi-runtime-compilers|2026.0.0-947|
|intel-oneapi-runtime-dpcpp-cpp|2026.0.0-947|
|intel-oneapi-runtime-opencl|2026.0.0-947|
|intel-opencl-icd|26.18.38308.1-1~24.04~ppa1|
|inxi|3.3.34-1-1|
|ipp-usb|0.9.24-0ubuntu3.3|
|iproute2|6.1.0-1ubuntu6.3|
|iptables|1.8.10-3ubuntu2|
|iputils-ping|3:20240117-1ubuntu0.1|
|iputils-tracepath|3:20240117-1ubuntu0.1|
|isc-dhcp-client|4.4.3-P1-4ubuntu2|
|isc-dhcp-common|4.4.3-P1-4ubuntu2|
|iso-codes|4.16.0-1|
|iso-flag-png|1.0.4|
|iucode-tool|2.3.1-3build1|
|jfsutils|1.1.15-5build4|
|jq|1.7.1-3ubuntu0.24.04.2|
|kbd|2.6.4-2ubuntu2|
|kerneloops|0.12+git20140509-6ubuntu8|
|keyboard-configuration|1.226ubuntu1|
|keyboxd|2.4.4-2ubuntu17.4|
|keyutils|1.6.3-3build1|
|kitty|0.32.2-1ubuntu0.4|
|klibc-utils|2.0.13-4ubuntu0.2|
|kmod|31+20240202-2ubuntu7.2|
|kpartx|0.9.4-5ubuntu8.1|
|kpartx-boot|0.9.4-5ubuntu8.1|
|krb5-locales|1.20.1-6ubuntu2.6|
|language-pack-en|1:24.04+20260127|
|language-pack-en-base|1:24.04+20260127|
|language-pack-gnome-en|1:24.04+20260127|
|language-pack-gnome-en-base|1:24.04+20260127|
|laptop-detect|0.16|
|less|590-2ubuntu2.1|
|libaa1|1.4p5-51.1|
|libaa1|1.4p5-51.1|
|libabsl20220623t64|20220623.1-3.1ubuntu3.2|
|libabw-0.1-1|0.1.3-1build4|
|libaccountsservice0|23.13.9-2ubuntu6|
|libacl1|2.3.2-1build1.1|
|libadwaita-1-0|1.5.0-2mint3+zara|
|libaio1t64|0.3.113-6build1.1|
|libao4|1.2.2+20180113-1.1ubuntu4|
|libao-common|1.2.2+20180113-1.1ubuntu4|
|libaom3|3.8.2-2ubuntu0.1|
|libaom3|3.8.2-2ubuntu0.1|
|libapksig-java|31.0.2-1ubuntu1|
|libapparmor1|4.0.1really4.0.1-0ubuntu0.24.04.7|
|libapparmor1|4.0.1really4.0.1-0ubuntu0.24.04.7|
|libappstream5|1.0.2-1build6|
|libapt-pkg6.0t64|2.8.3|
|libaquamarine11|0.12.0-1ppa1|
|libarchive13t64|3.7.2-2ubuntu0.7|
|libarchive-tools|3.7.2-2ubuntu0.7|
|libargon2-1|0~20190702+dfsg-4build1|
|libasan8|16-20260315-1ubuntu1~24~ppa1|
|libasound2-data|1.2.11-1ubuntu0.2|
|libasound2-dev|1.2.11-1ubuntu0.2|
|libasound2t64|1.2.11-1ubuntu0.2|
|libasound2t64|1.2.11-1ubuntu0.2|
|libaspell15|0.60.8.1-1build1|
|libass9|1:0.17.1-2build1|
|libassuan0|2.5.6-1build1|
|libasyncns0|0.8-6build4|
|libasyncns0|0.8-6build4|
|libatasmart4|0.19-5build3|
|libatk1.0-0t64|2.52.0-1build1|
|libatk1.0-0t64|2.52.0-1build1|
|libatk-adaptor|2.52.0-1build1|
|libatk-bridge2.0-0t64|2.52.0-1build1|
|libatk-bridge2.0-0t64|2.52.0-1build1|
|libatkmm-1.6-1v5|2.28.4-1build4|
|libatm1t64|1:2.5.1-5.1build1|
|libatomic1|16-20260315-1ubuntu1~24~ppa1|
|libatomic1|16-20260315-1ubuntu1~24~ppa1|
|libatopology2t64|1.2.11-1ubuntu0.2|
|libatspi2.0-0t64|2.52.0-1build1|
|libatspi2.0-0t64|2.52.0-1build1|
|libattr1|1:2.5.2-1build1.1|
|libaudit1|1:3.1.2-2.1build1.1|
|libaudit-common|1:3.1.2-2.1build1.1|
|libauthen-sasl-perl|2.1700-1|
|libavahi-client3|0.8-13ubuntu6.2|
|libavahi-client3|0.8-13ubuntu6.2|
|libavahi-common3|0.8-13ubuntu6.2|
|libavahi-common3|0.8-13ubuntu6.2|
|libavahi-common-data|0.8-13ubuntu6.2|
|libavahi-common-data|0.8-13ubuntu6.2|
|libavahi-core7|0.8-13ubuntu6.2|
|libavahi-glib1|0.8-13ubuntu6.2|
|libavc1394-0|0.5.4-5build3|
|libavc1394-0|0.5.4-5build3|
|libavcodec-dev|7:6.1.1-3ubuntu5|
|libavdevice60|7:6.1.1-3ubuntu5|
|libavfilter9|7:6.1.1-3ubuntu5|
|libavformat60|7:6.1.1-3ubuntu5|
|libavformat-dev|7:6.1.1-3ubuntu5|
|libavutil58|7:6.1.1-3ubuntu5|
|libavutil-dev|7:6.1.1-3ubuntu5|
|libayatana-appindicator3-1|0.5.93-1build3|
|libayatana-ido3-0.4-0|0.10.1-1build2|
|libayatana-indicator3-7|0.9.4-1build1|
|libbabeltrace1|1.5.11-3build3|
|libbinutils|2.42-4ubuntu2.10|
|libblas3|3.12.0-3build1.1|
|libblkid1|2.39.3-9ubuntu6.5|
|libblkid1|2.39.3-9ubuntu6.5|
|libblockdev3|3.1.1-1ubuntu0.1|
|libblockdev-crypto3|3.1.1-1ubuntu0.1|
|libblockdev-fs3|3.1.1-1ubuntu0.1|
|libblockdev-loop3|3.1.1-1ubuntu0.1|
|libblockdev-mdraid3|3.1.1-1ubuntu0.1|
|libblockdev-nvme3|3.1.1-1ubuntu0.1|
|libblockdev-part3|3.1.1-1ubuntu0.1|
|libblockdev-swap3|3.1.1-1ubuntu0.1|
|libblockdev-utils3|3.1.1-1ubuntu0.1|
|libbluetooth3|5.72-0ubuntu5.5|
|libbluetooth-dev|5.72-0ubuntu5.5|
|libbluray2|1:1.3.4-1build1|
|libboost-iostreams1.83.0|1.83.0-2.1ubuntu3.2|
|libboost-locale1.83.0|1.83.0-2.1ubuntu3.2|
|libboost-thread1.83.0|1.83.0-2.1ubuntu3.2|
|libbpf1|1:1.3.0-2build2|
|libbpfcc|0.29.1+ds-1ubuntu7|
|libbrasero-media3-1|3.12.3-2build4|
|libbrlapi0.8|6.6-4ubuntu5|
|libbrotli1|1.1.0-2build2|
|libbrotli1|1.1.0-2build2|
|libbs2b0|3.1.0+dfsg-7build1|
|libbsd0|0.12.1-1build1.1|
|libbsd0|0.12.1-1build1.1|
|libburn4t64|1.5.6-1.1build1|
|libbytesize1|2.10-1ubuntu2|
|libbytesize-common|2.10-1ubuntu2|
|libbz2-1.0|1.0.8-5.1build0.1|
|libbz2-1.0|1.0.8-5.1build0.1|
|libc6|2.39-0ubuntu8.7|
|libc6|2.39-0ubuntu8.7|
|libc6-dbg|2.39-0ubuntu8.7|
|libc6-dev|2.39-0ubuntu8.7|
|libc++abi-dev|1:18.0-59~exp2|
|libcaca0|0.99.beta20-4ubuntu0.2|
|libcaca0|0.99.beta20-4ubuntu0.2|
|libcairo2|1.18.0-3build1|
|libcairo2|1.18.0-3build1|
|libcairo2-dev|1.18.0-3build1|
|libcairo-5c-dev|1.21build1|
|libcairo-gobject2|1.18.0-3build1|
|libcairo-gobject2|1.18.0-3build1|
|libcairo-gobject-perl|1.005-4build3|
|libcairomm-1.0-1v5|1.14.5-1build1|
|libcairomm-1.16-1|1.18.0-1build1|
|libcairo-perl|1.109-4build1|
|libcairo-script-interpreter2|1.18.0-3build1|
|libcamel-1.2-64t64|3.52.3-0ubuntu1.2|
|libcanberra0t64|0.30-10ubuntu10|
|libcanberra-dev|0.30-10ubuntu10|
|libcanberra-gtk3-0t64|0.30-10ubuntu10|
|libcanberra-gtk3-module|0.30-10ubuntu10|
|libcanberra-pulse|0.30-10ubuntu10|
|libcap2|1:2.66-5ubuntu2.4|
|libcap2|1:2.66-5ubuntu2.4|
|libcap2-bin|1:2.66-5ubuntu2.4|
|libcap-ng0|0.8.4-2build2|
|libcaribou0|0.4.21-9build1|
|libcaribou-common|0.4.21-9build1|
|libc-bin|2.39-0ubuntu8.7|
|libcbor0.10|0.10.2-1.2ubuntu2|
|libcc1-0|16-20260315-1ubuntu1~24~ppa1|
|libc++-dev|1:18.0-59~exp2|
|libc-dev-bin|2.39-0ubuntu8.7|
|libcdio19t64|2.1.0-4.1ubuntu1.2|
|libcdio-cdda2t64|10.2+2.0.1-1.1build2|
|libcdio-paranoia2t64|10.2+2.0.1-1.1build2|
|libcdparanoia0|3.10.2+debian-14build3|
|libcdparanoia0|3.10.2+debian-14build3|
|libcdr-0.1-1|0.1.7-1build2|
|libcgi-fast-perl|1:2.17-1|
|libcgi-pm-perl|4.63-1|
|libchamplain-0.12-0|0.12.21-1build3|
|libchamplain-gtk-0.12-0|0.12.21-1build3|
|libchromaprint1|1.5.1-5|
|libcinnamon-control-center1|6.6.0+zena|
|libcinnamon-desktop4|6.6.2+zena|
|libcinnamon-desktop-dbg|6.6.2+zena|
|libcinnamon-menu-3-0|6.6.0+zena|
|libcinnamon-menu-3-0-dbg|6.6.0+zena|
|libcjs0|115.1+zena|
|libcjs-dbg|115.1+zena|
|libcjson1|1.7.17-1|
|libclone-perl|0.46-1build3|
|libclucene-contribs1t64|2.3.3.4+dfsg-1.2ubuntu2|
|libclucene-core1t64|2.3.3.4+dfsg-1.2ubuntu2|
|libclutter-1.0-0|1.26.4+git2779b932+dfsg-7build3|
|libclutter-1.0-common|1.26.4+git2779b932+dfsg-7build3|
|libclutter-gst-3.0-0|3.0.27-4build1|
|libclutter-gtk-1.0-0|1.8.4-4build5|
|libcodec2-1.2|1.2.0-2build1|
|libcogl20|1.22.8-4ubuntu4|
|libcogl-common|1.22.8-4ubuntu4|
|libcogl-pango20|1.22.8-4ubuntu4|
|libcogl-path20|1.22.8-4ubuntu4|
|libcolamd3|1:7.6.1+dfsg-1build1|
|libcolord2|1.4.7-1build2|
|libcolord2|1.4.7-1build2|
|libcolorhug2|1.4.7-1build2|
|libcom-err2|1.47.0-2.4~exp1ubuntu4.1|
|libcom-err2|1.47.0-2.4~exp1ubuntu4.1|
|libcpdb2t64|2.0~b5-1.2build1|
|libcrack2|2.9.6-5.1build2|
|libcrypt1|1:4.4.36-4build1|
|libcrypt1|1:4.4.36-4build1|
|libcrypt-dev|1:4.4.36-4build1|
|libcryptsetup12|2:2.7.0-1ubuntu4.2|
|libctf0|2.42-4ubuntu2.10|
|libctf-nobfd0|2.42-4ubuntu2.10|
|libcups2t64|2.4.7-1.2ubuntu7.13|
|libcups2t64|2.4.7-1.2ubuntu7.13|
|libcupsfilters2-common|2.0.0-0ubuntu7.2|
|libcupsfilters2t64|2.0.0-0ubuntu7.2|
|libcupsimage2t64|2.4.7-1.2ubuntu7.13|
|libcurl3t64-gnutls|8.5.0-2ubuntu10.9|
|libcurl3t64-gnutls|8.5.0-2ubuntu10.9|
|libcurl4t64|8.5.0-2ubuntu10.9|
|libcurl4t64|8.5.0-2ubuntu10.9|
|libcvc0|6.6.2+zena|
|libcvc-dbg|6.6.2+zena|
|libcwidget4|0.5.18-6build1|
|libdaemon0|0.14-7.1ubuntu4|
|libdata-dump-perl|1.25-1|
|libdatrie1|0.2.13-3build1|
|libdatrie1|0.2.13-3build1|
|libdav1d7|1.4.1-1build1|
|libdazzle-1.0-0|3.44.0-1build3|
|libdazzle-common|3.44.0-1build3|
|libdb5.3t64|5.3.28+dfsg2-7|
|libdb5.3t64|5.3.28+dfsg2-7|
|libdbus-1-3|1.14.10-4ubuntu4.1|
|libdbus-1-3|1.14.10-4ubuntu4.1|
|libdbus-glib-1-2|0.112-3build2|
|libdbusmenu-glib4|18.10.20180917~bzr492+repack1-3.1ubuntu5|
|libdbusmenu-gtk3-4|18.10.20180917~bzr492+repack1-3.1ubuntu5|
|libdc1394-25|2.2.6-4build1|
|libdconf1|0.40.0-4ubuntu0.1|
|libde265-0|1.0.15-1build3|
|libde265-0|1.0.15-1build3|
|libdebconfclient0|0.271ubuntu3|
|libdebuginfod1t64|0.190-1.1ubuntu0.1|
|libdebuginfod-common|0.190-1.1ubuntu0.1|
|libdecor-0-0|0.2.2-1build2|
|libdecor-0-0|0.2.2-1build2|
|libdecor-0-dev|0.2.2-1build2|
|libdee-1.0-4|1.2.7+17.10.20170616-7build5|
|libdeflate0|1.19-1build1.1|
|libdeflate0|1.19-1build1.1|
|libdevmapper1.02.1|2:1.02.185-3ubuntu3.2|
|libdevmapper-event1.02.1|2:1.02.185-3ubuntu3.2|
|libdjvulibre21|3.5.28-2ubuntu0.24.04.2|
|libdjvulibre-text|3.5.28-2ubuntu0.24.04.2|
|libdmapsharing-4.0-3t64|3.9.13-2build1|
|libdmraid1.0.0.rc16|1.0.0.rc16-12ubuntu2|
|libdotconf0|1.3-0.3fakesync1build3|
|libdouble-conversion3|3.3.0-1build1|
|libdpkg-perl|1.22.6ubuntu6.6|
|libdrm2|2.4.125-1ubuntu0.1~24.04.1|
|libdrm2|2.4.125-1ubuntu0.1~24.04.1|
|libdrm-amdgpu1|2.4.125-1ubuntu0.1~24.04.1|
|libdrm-amdgpu1|2.4.125-1ubuntu0.1~24.04.1|
|libdrm-common|2.4.125-1ubuntu0.1~24.04.1|
|libdrm-dev|2.4.125-1ubuntu0.1~24.04.1|
|libdrm-intel1|2.4.125-1ubuntu0.1~24.04.1|
|libdrm-intel1|2.4.125-1ubuntu0.1~24.04.1|
|libdrm-radeon1|2.4.125-1ubuntu0.1~24.04.1|
|libduktape207|2.7.0+tests-0ubuntu3|
|libduktape207|2.7.0+tests-0ubuntu3|
|libdv4t64|1.0.0-17.1build1|
|libdv4t64|1.0.0-17.1build1|
|libdvdnav4|6.1.1-3build1|
|libdvdread8t64|6.1.3-1.1build1|
|libdw1t64|0.190-1.1ubuntu0.1|
|libdw1t64|0.190-1.1ubuntu0.1|
|libdw-dev|0.190-1.1ubuntu0.1|
|libebackend-1.2-11t64|3.52.3-0ubuntu1.2|
|libe-book-0.1-1|0.1.3-2build6|
|libebook-1.2-21t64|3.52.3-0ubuntu1.2|
|libebook-contacts-1.2-4t64|3.52.3-0ubuntu1.2|
|libecal-2.0-3|3.52.3-0ubuntu1.2|
|libecryptfs1|111-6ubuntu3|
|libedata-book-1.2-27t64|3.52.3-0ubuntu1.2|
|libedata-cal-2.0-2t64|3.52.3-0ubuntu1.2|
|libedataserver-1.2-27t64|3.52.3-0ubuntu1.2|
|libedataserverui-1.2-4t64|3.52.3-0ubuntu1.2|
|libedit2|3.1-20230828-1build1|
|libedit2|3.1-20230828-1build1|
|libefiboot1t64|38-3.1build1|
|libefivar1t64|38-3.1build1|
|libegl1|1.7.0-1build1|
|libegl1|1.7.0-1build1|
|libegl1-mesa-dev|26.1.2~kisak1~n|
|libegl-dev|1.7.0-1build1|
|libegl-mesa0|26.1.2~kisak1~n|
|libegl-mesa0|26.1.2~kisak1~n|
|libeigen3-dev|3.4.0-4build0.1|
|libelf1t64|0.190-1.1ubuntu0.1|
|libelf1t64|0.190-1.1ubuntu0.1|
|libenchant-2-2|2.3.3-2build2|
|libencode-locale-perl|1.05-3|
|libeot0|0.01-5build3|
|libepoxy0|1.5.10-1build1|
|libepoxy0|1.5.10-1build1|
|libept1.6.0t64|1.2.1+nmu1build1|
|libepubgen-0.1-1|0.1.1-1ubuntu6|
|libespeak-ng1|1.51+dfsg-12build1|
|libestr0|0.1.11-1build1|
|libetonyek-0.1-1|0.1.10-5build1|
|libevdev2|1.13.1+dfsg-1build1|
|libevent-2.1-7t64|2.1.12-stable-9ubuntu2|
|libexempi8|2.6.5-1build1|
|libexif12|0.6.24-1build2|
|libexif12|0.6.24-1build2|
|libexiv2-27|0.27.6-1ubuntu0.3|
|libexpat1|2.6.1-2ubuntu0.4|
|libexpat1|2.6.1-2ubuntu0.4|
|libext2fs2t64|1.47.0-2.4~exp1ubuntu4.1|
|libexttextcat-2.0-0|3.4.7-1build1|
|libexttextcat-data|3.4.7-1build1|
|libextutils-depends-perl|0.8001-2|
|libfakeroot|1.33-1|
|libfastjson4|1.2304.0-1build1|
|libfcgi0t64|2.4.2-2.1ubuntu0.24.04.1|
|libfcgi-perl|0.82+ds-3build2|
|libfdisk1|2.39.3-9ubuntu6.5|
|libffi8|3.4.6-1build1|
|libffi8|3.4.6-1build1|
|libffi-dev|3.4.6-1build1|
|libffmpegthumbnailer4v5|2.2.2+git20240104+dfsg-1build1|
|libfftw3-double3|3.3.10-1ubuntu3|
|libfftw3-single3|3.3.10-1ubuntu3|
|libfido2-1|1.14.0-1build3|
|libfile-basedir-perl|0.09-2|
|libfile-desktopentry-perl|0.22-3|
|libfile-fcntllock-perl|0.22-4ubuntu5|
|libfile-listing-perl|6.16-1|
|libfile-mimeinfo-perl|0.34-1|
|libflac12t64|1.4.3+ds-2.1ubuntu2|
|libflac12t64|1.4.3+ds-2.1ubuntu2|
|libflashrom1|1.3.0-2.1ubuntu2|
|libflatpak0|1.14.6-1ubuntu0.1|
|libflite1|2.2-6build3|
|libfont-afm-perl|1.20-4|
|libfontconfig1|2.15.0-1.1ubuntu2|
|libfontconfig1|2.15.0-1.1ubuntu2|
|libfontconfig1-dev|2.15.0-1.1ubuntu2|
|libfontconfig-dev|2.15.0-1.1ubuntu2|
|libfontenc1|1:1.1.8-1build1|
|libfprint-2-2|1:1.94.7+tod1-0ubuntu5~24.04.6|
|libfprint-2-tod1|1:1.94.7+tod1-0ubuntu5~24.04.6|
|libfreeaptx0|0.1.1-2build1|
|libfreehand-0.1-1|0.1.2-3build3|
|libfreetype6|2.13.2+dfsg-1ubuntu0.1|
|libfreetype6|2.13.2+dfsg-1ubuntu0.1|
|libfribidi0|1.0.13-3build1|
|libfribidi0|1.0.13-3build1|
|libfribidi-dev|1.0.13-3build1|
|libftdi1-2|1.5-6build5|
|libfuse2t64|2.9.9-8.1build1|
|libfuse3-3|3.14.0-5build1|
|libfwupd2|1.9.34-0ubuntu1~24.04.1|
|libgail18t64|2.24.33-4ubuntu1.1|
|libgail-3-0t64|3.24.41-4ubuntu1.3|
|libgail-common|2.24.33-4ubuntu1.1|
|libgamemode0|1.8.1-2build1|
|libgamemodeauto0|1.8.1-2build1|
|libgbm1|26.1.2~kisak1~n|
|libgbm1|26.1.2~kisak1~n|
|libgbm-dev|26.1.2~kisak1~n|
|libgcc-13-dev|13.3.0-6ubuntu2~24.04.1|
|libgcc-s1|16-20260315-1ubuntu1~24~ppa1|
|libgcc-s1|16-20260315-1ubuntu1~24~ppa1|
|libgck-1-0|3.41.2-1build3|
|libgck-2-2|4.2.0-5|
|libgcr-4-4|4.2.0-5|
|libgcr-base-3-1|3.41.2-1build3|
|libgcr-ui-3-1|3.41.2-1build3|
|libgcrypt20|1.10.3-2ubuntu0.1|
|libgcrypt20|1.10.3-2ubuntu0.1|
|libgcrypt20-dev|1.10.3-2ubuntu0.1|
|libgd3|2.3.3-9ubuntu5|
|libgd3|2.3.3-9ubuntu5|
|libgdata22|0.18.1-6build2|
|libgdata-common|0.18.1-6build2|
|libgdbm6t64|1.23-5.1build1|
|libgdbm-compat4t64|1.23-5.1build1|
|libgdk-pixbuf-2.0-0|2.42.10+dfsg-3ubuntu3.3|
|libgdk-pixbuf-2.0-0|2.42.10+dfsg-3ubuntu3.3|
|libgdk-pixbuf2.0-bin|2.42.10+dfsg-3ubuntu3.3|
|libgdk-pixbuf2.0-common|2.42.10+dfsg-3ubuntu3.3|
|libgee-0.8-2|0.20.6-1build2|
|libgeocode-glib-2-0|3.26.3-6build3|
|libgexiv2-2|0.14.2-2build3|
|libgfortran5|16-20260315-1ubuntu1~24~ppa1|
|libgif7|5.2.2-1ubuntu1|
|libgirepository-1.0-1|1.80.1-1|
|libgirepository1.0-dev|1.80.1-1|
|libgl1|1.7.0-1build1|
|libgl1|1.7.0-1build1|
|libgl1-mesa-dev|26.1.2~kisak1~n|
|libgl1-mesa-dri|26.1.2~kisak1~n|
|libgl1-mesa-dri|26.1.2~kisak1~n|
|libgles2|1.7.0-1build1|
|libgles2-mesa-dev|26.1.2~kisak1~n|
|libglfw3-dev|3.3.10-1build1|
|libglfw3-wayland|3.3.10-1build1|
|libglib2.0-0t64|2.80.0-6ubuntu3.8|
|libglib2.0-0t64|2.80.0-6ubuntu3.8|
|libglib2.0-bin|2.80.0-6ubuntu3.8|
|libglib2.0-data|2.80.0-6ubuntu3.8|
|libglib2.0-dev|2.80.0-6ubuntu3.8|
|libglibmm-2.4-1t64|2.66.7-1build1|
|libglibmm-2.68-1t64|2.78.1-2.2build2|
|libglib-object-introspection-perl|0.051-1build3|
|libglib-perl|3:1.329.3-3build3|
|libglm-dev|0.9.9.8+ds-7|
|libglu1-mesa|9.0.2-1.1build1|
|libglvnd0|1.7.0-1build1|
|libglvnd0|1.7.0-1build1|
|libglx0|1.7.0-1build1|
|libglx0|1.7.0-1build1|
|libglx-mesa0|26.1.2~kisak1~n|
|libglx-mesa0|26.1.2~kisak1~n|
|libgme0|0.6.3-7build1|
|libgmp10|2:6.3.0+dfsg-2ubuntu6.1|
|libgmp10|2:6.3.0+dfsg-2ubuntu6.1|
|libgmp-dev|2:6.3.0+dfsg-2ubuntu6.1|
|libgnome-desktop-3-20t64|44.0-5build2|
|libgnomekbd8|3.28.1-1build3|
|libgnomekbd-common|3.28.1-1build3|
|libgnutls30t64|3.8.3-1.1ubuntu3.6|
|libgnutls30t64|3.8.3-1.1ubuntu3.6|
|libgoa-1.0-0b|3.50.4-0ubuntu2|
|libgoa-1.0-common|3.50.4-0ubuntu2|
|libgoa-backend-1.0-2|3.50.4-0ubuntu2|
|libgomp1|16-20260315-1ubuntu1~24~ppa1|
|libgpg-error0|1.47-3build2.1|
|libgpg-error0|1.47-3build2.1|
|libgpgme11t64|1.18.0-4.1ubuntu4|
|libgpgmepp6t64|1.18.0-4.1ubuntu4|
|libgphoto2-6t64|2.5.31-2.1ubuntu1|
|libgphoto2-6t64|2.5.31-2.1ubuntu1|
|libgphoto2-l10n|2.5.31-2.1ubuntu1|
|libgphoto2-port12t64|2.5.31-2.1ubuntu1|
|libgphoto2-port12t64|2.5.31-2.1ubuntu1|
|libgpm2|1.20.7-11|
|libgpm2|1.20.7-11|
|libgpod4t64|0.8.3-19.1ubuntu4|
|libgpod-common|0.8.3-19.1ubuntu4|
|libgprofng0|2.42-4ubuntu2.10|
|libgraphene-1.0-0|1.10.8-3build2|
|libgraphene-1.0-dev|1.10.8-3build2|
|libgraphite2-3|1.3.14-2build1|
|libgraphite2-3|1.3.14-2build1|
|libgrilo-0.3-0|0.3.16-1build3|
|libgs10|10.02.1~dfsg1-0ubuntu7.8|
|libgs10-common|10.02.1~dfsg1-0ubuntu7.8|
|libgs-common|10.02.1~dfsg1-0ubuntu7.8|
|libgsf-1-114|1.14.51-2ubuntu0.1|
|libgsf-1-common|1.14.51-2ubuntu0.1|
|libgsf-bin|1.14.51-2ubuntu0.1|
|libgsm1|1.0.22-1build1|
|libgsound0t64|1.0.3-3.2build2|
|libgspell-1-2|1.12.2-1build4|
|libgspell-1-common|1.12.2-1build4|
|libgssapi-krb5-2|1.20.1-6ubuntu2.6|
|libgssapi-krb5-2|1.20.1-6ubuntu2.6|
|libgstreamer1.0-0|1.24.2-1ubuntu0.1|
|libgstreamer1.0-0|1.24.2-1ubuntu0.1|
|libgstreamer-gl1.0-0|1.24.2-1ubuntu0.4|
|libgstreamer-plugins-base1.0-0|1.24.2-1ubuntu0.4|
|libgstreamer-plugins-base1.0-0|1.24.2-1ubuntu0.4|
|libgstreamer-plugins-good1.0-0|1.24.2-1ubuntu1.4|
|libgstreamer-plugins-good1.0-0|1.24.2-1ubuntu1.4|
|libgtk2.0-0t64|2.24.33-4ubuntu1.1|
|libgtk2.0-bin|2.24.33-4ubuntu1.1|
|libgtk2.0-common|2.24.33-4ubuntu1.1|
|libgtk-3-0t64|3.24.41-4ubuntu1.3|
|libgtk-3-0t64|3.24.41-4ubuntu1.3|
|libgtk-3-bin|3.24.41-4ubuntu1.3|
|libgtk-3-common|3.24.41-4ubuntu1.3|
|libgtk-3-dev|3.24.41-4ubuntu1.3|
|libgtk3-perl|0.038-3|
|libgtk-4-1|4.14.5+ds-0ubuntu0.10|
|libgtk-4-common|4.14.5+ds-0ubuntu0.10|
|libgtk-layer-shell0|0.9.0-1ppa1|
|libgtkmm-3.0-1t64|3.24.9-1|
|libgtkmm-4.0-0|4.10.0-4build3|
|libgtksourceview-3.0-1|3.24.11-2build4|
|libgtksourceview-3.0-common|3.24.11-2build4|
|libgtksourceview-4-0|4.8.4-5build4|
|libgtksourceview-4-common|4.8.4-5build4|
|libgtop-2.0-11|2.41.3-1ubuntu0.24.04.1|
|libgtop2-common|2.41.3-1ubuntu0.24.04.1|
|libgucharmap-2-90-7|1:15.1.3-1build1|
|libgudev-1.0-0|1:238-5ubuntu1|
|libgudev-1.0-0|1:238-5ubuntu1|
|libgudev-1.0-dev|1:238-5ubuntu1|
|libgusb2|0.4.8-1build2|
|libgutenprint9|5.3.4.20220624T01008808d602-1build4|
|libgutenprint-common|5.3.4.20220624T01008808d602-1build4|
|libgweather-4-0t64|4.4.2-1build1|
|libgweather-4-common|4.4.2-1build1|
|libgxps2t64|0.3.2-4build3|
|libhandy-1-0|1.8.3-1build2|
|libharfbuzz0b|8.3.0-2build2|
|libharfbuzz0b|8.3.0-2build2|
|libharfbuzz-gobject0|8.3.0-2build2|
|libharfbuzz-icu0|8.3.0-2build2|
|libheif1|1.17.6-1ubuntu4.3|
|libheif1|1.17.6-1ubuntu4.3|
|libheif-plugin-aomdec|1.17.6-1ubuntu4.3|
|libheif-plugin-aomdec|1.17.6-1ubuntu4.3|
|libheif-plugin-libde265|1.17.6-1ubuntu4.3|
|libheif-plugin-libde265|1.17.6-1ubuntu4.3|
|libhogweed6t64|3.9.1-2.2build1.1|
|libhogweed6t64|3.9.1-2.2build1.1|
|libhpmud0|3.23.12+dfsg0-0ubuntu5|
|libhtml-format-perl|2.16-2|
|libhtml-form-perl|6.11-1|
|libhtml-parser-perl|3.81-1build3|
|libhtml-tagset-perl|3.20-6|
|libhtml-tree-perl|5.07-3|
|libhttp-cookies-perl|6.11-1|
|libhttp-daemon-perl|6.16-1ubuntu0.24.04.1|
|libhttp-date-perl|6.06-1|
|libhttp-message-perl|6.45-1ubuntu1|
|libhttp-negotiate-perl|6.01-2|
|libhunspell-1.7-0|1.7.2+really1.7.2-10build3|
|libhwasan0|16-20260315-1ubuntu1~24~ppa1|
|libhwy1t64|1.0.7-8.1build1|
|libhyphen0|2.8.8-7build3|
|libhyprutils11|0.12.0-1ppa1|
|libhyprwire3|0.3.1-1ppa3|
|libibus-1.0-5|1.5.29-2|
|libibus-1.0-dev|1.5.29-2|
|libibverbs1|50.0-2ubuntu0.2|
|libical3t64|3.0.17-1.1build3|
|libice6|2:1.0.10-1build3|
|libice-dev|2:1.0.10-1build3|
|libicu74|74.2-1ubuntu3.1|
|libicu74|74.2-1ubuntu3.1|
|libidn12|1.42-1build1|
|libidn2-0|2.3.7-2build1.1|
|libidn2-0|2.3.7-2build1.1|
|libiec61883-0|1.2.0-6build1|
|libiec61883-0|1.2.0-6build1|
|libieee1284-3t64|0.2.11-14.1build1|
|libigc2|2.34.4-1260~24.04|
|libigdgmm12|22.10.0-1~24.04~ppa1|
|libigdgmm12|22.10.0-1~24.04~ppa1|
|libigdgmm-dev|22.10.0-1~24.04~ppa1|
|libijs-0.35|0.35-15.1build1|
|libimagequant0|2.18.0-1build1|
|libimath-3-1-29t64|3.1.9-3.1ubuntu2|
|libimobiledevice6|1.3.0-8.1build3|
|libimobiledevice-utils|1.3.0-8.1build3|
|libinih1|55-1ubuntu2|
|libinih-dev|55-1ubuntu2|
|libinput10|1.31.1-1|
|libinput-bin|1.31.1-1|
|libinput-dev|1.31.1-1|
|libio-html-perl|1.004-3|
|libio-socket-ssl-perl|2.085-1|
|libip4tc2|1.8.10-3ubuntu2|
|libip6tc2|1.8.10-3ubuntu2|
|libipc-system-simple-perl|1.30-2|
|libipt2|2.0.6-1build1|
|libisl23|0.26-3build1.1|
|libisofs6t64|1.5.6.pl01-1.1ubuntu2|
|libitm1|16-20260315-1ubuntu1~24~ppa1|
|libiw30t64|30~pre9-16.1ubuntu2|
|libjansson4|2.14-2build2|
|libjavascriptcoregtk-4.1-0|2.52.3-0ubuntu0.24.04.1|
|libjbig0|2.1-6.1ubuntu2|
|libjbig0|2.1-6.1ubuntu2|
|libjbig2dec0|0.20-1build3|
|libjcat1|0.2.3-1~ubuntu0.24.04.1|
|libjpeg8|8c-2ubuntu11|
|libjpeg8|8c-2ubuntu11|
|libjpeg-turbo8|2.1.5-2ubuntu2|
|libjpeg-turbo8|2.1.5-2ubuntu2|
|libjs-mathjax|2.7.9+dfsg-1|
|libjson-c5|0.17-1build1|
|libjson-glib-1.0-0|1.8.0-2build2|
|libjson-glib-1.0-common|1.8.0-2build2|
|libjson-glib-dev|1.8.0-2build2|
|libjxl0.7|0.7.0-10.2ubuntu6.1|
|libjxl-tools|0.7.0-10.2ubuntu6.1|
|libk5crypto3|1.20.1-6ubuntu2.6|
|libk5crypto3|1.20.1-6ubuntu2.6|
|libkeybinder-3.0-0|0.3.2-1.1build2|
|libkeyutils1|1.6.3-3build1|
|libkeyutils1|1.6.3-3build1|
|libklibc|2.0.13-4ubuntu0.2|
|libkmod2|31+20240202-2ubuntu7.2|
|libkmod-dev|31+20240202-2ubuntu7.2|
|libkpathsea6|2023.20230311.66589-9build3|
|libkrb5-3|1.20.1-6ubuntu2.6|
|libkrb5-3|1.20.1-6ubuntu2.6|
|libkrb5-dev|1.20.1-6ubuntu2.6|
|libkrb5support0|1.20.1-6ubuntu2.6|
|libkrb5support0|1.20.1-6ubuntu2.6|
|libksba8|1.6.6-1build1|
|liblangtag1|0.6.7-1build2|
|liblangtag-common|0.6.7-1build2|
|liblapack3|3.12.0-3build1.1|
|liblc3-1|1.0.4-3build1|
|liblcms2-2|2.14-2ubuntu0.1|
|liblcms2-2|2.14-2ubuntu0.1|
|liblcms2-utils|2.14-2ubuntu0.1|
|libldacbt-abr2|2.0.2.3+git20200429+ed310a0-4ubuntu2|
|libldacbt-enc2|2.0.2.3+git20200429+ed310a0-4ubuntu2|
|libldap2|2.6.10+dfsg-0ubuntu0.24.04.1|
|libldap2|2.6.10+dfsg-0ubuntu0.24.04.1|
|libldb2|2:2.8.0+samba4.19.5+dfsg-4ubuntu9.6|
|liblerc4|4.0.0+ds-4ubuntu2|
|liblightdm-gobject-1-0|1.30.0-0ubuntu14|
|liblilv-0-0|0.24.22-1build1|
|liblirc-client0t64|0.10.2-0.8build1|
|libllvm17t64|1:17.0.6-9ubuntu1|
|libllvm19|1:19.1.7-1ubuntu2~kisak~n|
|libllvm20|1:20.1.8-0ubuntu1~kisak~n|
|libllvm20|1:20.1.8-0ubuntu1~kisak~n|
|liblmdb0|0.9.31-1build1|
|liblocale-gettext-perl|1.07-6ubuntu5|
|liblocale-msgfmt-perl|0.15-2|
|liblouis20|3.29.0-1build1|
|liblouis-data|3.29.0-1build1|
|liblouisutdml9t64|2.12.0-3.1build1|
|liblouisutdml-bin|2.12.0-3.1build1|
|liblouisutdml-data|2.12.0-3.1build1|
|liblsan0|16-20260315-1ubuntu1~24~ppa1|
|libltdl7|2.4.7-7build1|
|libltdl7|2.4.7-7build1|
|liblua5.2-0|5.2.4-3build2|
|liblua5.4-0|5.4.6-3build2|
|liblvm2cmd2.03|2.03.16-3ubuntu3.2|
|liblwp-mediatypes-perl|6.04-2|
|liblwp-protocol-https-perl|6.13-1|
|liblz4-1|1.9.4-1build1.1|
|liblz4-1|1.9.4-1build1.1|
|liblz4-dev|1.9.4-1build1.1|
|liblzma5|5.6.1+really5.4.5-1ubuntu0.3|
|liblzma5|5.6.1+really5.4.5-1ubuntu0.3|
|liblzo2-2|2.10-2build4|
|libmagic1t64|1:5.45-3build1|
|libmagic-dev|1:5.45-3build1|
|libmagic-mgc|1:5.45-3build1|
|libmalcontent-0-0|0.11.1-1ubuntu1.3|
|libmanette-0.2-0|0.2.7-1build2|
|libmate-desktop-2-17t64|1.26.2-1.1mint1+zara|
|libmate-menu2|1.26.1+mint1|
|libmate-panel-applet-4-1|1.27.1-2build3|
|libmateweather1t64|1.26.3-3.1build2|
|libmateweather-common|1.26.3-3.1build2|
|libmaxminddb0|1.9.1-1build1|
|libmbedcrypto7t64|2.28.8-1|
|libmbim-glib4|1.31.2-0ubuntu3.1|
|libmbim-proxy|1.31.2-0ubuntu3.1|
|libmd0|1.1.0-2build1.1|
|libmd0|1.1.0-2build1.1|
|libmd4c0|0.4.8-1build1|
|libmetacity3|1:3.52.0-1build1|
|libmetee5|5.0.0-1~24.04~ppa2|
|libmfx-gen1|25.4.0-0ubuntu1~24.04~ppa1|
|libmgba|0.10.5|
|libmhash2|0.9.9.9-9build3|
|libminiupnpc17|2.2.6-1build2|
|libmm-glib0|1.23.4-0ubuntu2|
|libmnl0|1.0.5-2build1|
|libmount1|2.39.3-9ubuntu6.5|
|libmount1|2.39.3-9ubuntu6.5|
|libmozjs-115-0t64|115.10.0-1|
|libmp3lame0|3.100-6build1|
|libmp3lame0|3.100-6build1|
|libmpc3|1.3.1-1build1.1|
|libmpc-dev|1.3.1-1build1.1|
|libmpdclient-dev|2.22-1.1build1|
|libmpfr6|4.2.1-1build1.1|
|libmpfr-dev|4.2.1-1build1.1|
|libmpg123-0t64|1.32.5-1ubuntu1.1|
|libmpg123-0t64|1.32.5-1ubuntu1.1|
|libmpv2|0.37.0-1ubuntu4|
|libmsgraph-0-1|0.2.1-0ubuntu3|
|libmspub-0.1-1|0.1.4-3build7|
|libmtdev1t64|1.1.6-1.1build1|
|libmtp9t64|1.1.21-3.1ubuntu1|
|libmtp-common|1.1.21-3.1ubuntu1|
|libmtp-runtime|1.1.21-3.1ubuntu1|
|libmuffin0|6.6.3+zena|
|libmujs3|1.3.3-3build2|
|libmusicbrainz5-2|5.1.0+git20150707-10build2|
|libmusicbrainz5cc2v5|5.1.0+git20150707-10build2|
|libmwaw-0.3-3|0.3.22-1build1|
|libmysofa1|1.3.2+dfsg-2ubuntu2|
|libmythes-1.2-0|2:1.2.5-1build1|
|libnatpmp1t64|20230423-1.2build1|
|libnautilus-extension4|1:46.4-0ubuntu0.2|
|libncurses6|6.4+20240113-1ubuntu2|
|libncurses6|6.4+20240113-1ubuntu2|
|libncursesw6|6.4+20240113-1ubuntu2|
|libncursesw6|6.4+20240113-1ubuntu2|
|libndp0|1.8-1fakesync1ubuntu0.24.04.1|
|libnemo-extension1|6.6.3+zena|
|libneon27t64-gnutls|0.33.0-1.1build3|
|libnet-dbus-perl|1.2.0-2build3|
|libnetfilter-conntrack3|1.0.9-6build1|
|libnet-http-perl|6.23-1|
|libnet-libidn-perl|0.12.ds-4build4|
|libnetplan1|1.1.2-8ubuntu1~24.04.2|
|libnet-ssleay-perl|1.94-1build4|
|libnettle8t64|3.9.1-2.2build1.1|
|libnettle8t64|3.9.1-2.2build1.1|
|libnewt0.52|0.52.24-2ubuntu2|
|libnfnetlink0|1.0.2-2build1|
|libnfs14|5.0.2-1build1|
|libnftables1|1.0.9-1ubuntu0.1|
|libnftnl11|1.2.6-2build1|
|libnghttp2-14|1.59.0-1ubuntu0.3|
|libnghttp2-14|1.59.0-1ubuntu0.3|
|libnl-3-200|3.7.0-0.3build1.1|
|libnl-3-dev|3.7.0-0.3build1.1|
|libnl-genl-3-200|3.7.0-0.3build1.1|
|libnl-route-3-200|3.7.0-0.3build1.1|
|libnm0|1.46.0-1ubuntu2.7|
|libnm0|1.46.0-1ubuntu2.7|
|libnma0|1.10.6+mint1+wilma|
|libnma-common|1.10.6+mint1+wilma|
|libnma-gtk4-0|1.10.6+mint1+wilma|
|libnorm1t64|1.5.9+dfsg-3.1build1|
|libnotify4|0.8.3-1build2|
|libnotify-bin|0.8.3-1build2|
|libnpth0t64|1.6-3.1build1|
|libnspr4|2:4.35-1.1build1|
|libnss3|2:3.98-1ubuntu0.1|
|libnss-mdns|0.15.1-4build1|
|libnss-myhostname|255.4-1ubuntu8.16|
|libnss-systemd|255.4-1ubuntu8.16|
|libntfs-3g89t64|1:2022.10.3-1.2ubuntu3.1|
|libnuma1|2.0.18-1ubuntu0.24.04.1|
|libnvme1t64|1.8-3ubuntu1|
|libnvpair3linux|2.2.2-0ubuntu9.4|
|libodfgen-0.1-1|0.1.8-2build3|
|libogg0|1.3.5-3build1|
|libogg0|1.3.5-3build1|
|libopenal1|1:1.23.1-4build1|
|libopenal-data|1:1.23.1-4build1|
|libopenexr-3-1-30|3.1.5-5.1build3|
|libopengl0|1.7.0-1build1|
|libopenjp2-7|2.5.0-2ubuntu0.5|
|libopenmpt0t64|0.7.3-1.1build3|
|libopus0|1.4-1build1|
|libopus0|1.4-1build1|
|liborc-0.4-0t64|1:0.4.38-1ubuntu0.1|
|liborc-0.4-0t64|1:0.4.38-1ubuntu0.1|
|liborcus-0.18-0|0.19.2-3build3|
|liborcus-parser-0.18-0|0.19.2-3build3|
|libostree-1-1|2024.5-1build2|
|libp11-kit0|0.25.3-4ubuntu2.1|
|libp11-kit0|0.25.3-4ubuntu2.1|
|libpackagekit-glib2-18|1.2.8+mint3+wilma|
|libpagemaker-0.0-0|0.0.4-1build4|
|libpam0g|1.5.3-5ubuntu5.5|
|libpam0g-dev|1.5.3-5ubuntu5.5|
|libpam-cap|1:2.66-5ubuntu2.4|
|libpam-fingwit|1.0.8+zena|
|libpam-fprintd|1.94.3-1|
|libpam-gnome-keyring|46.1-2ubuntu0.2|
|libpam-modules|1.5.3-5ubuntu5.5|
|libpam-modules-bin|1.5.3-5ubuntu5.5|
|libpam-runtime|1.5.3-5ubuntu5.5|
|libpam-systemd|255.4-1ubuntu8.16|
|libpango-1.0-0|1.52.1+ds-1build1|
|libpango-1.0-0|1.52.1+ds-1build1|
|libpango1.0-dev|1.52.1+ds-1build1|
|libpangocairo-1.0-0|1.52.1+ds-1build1|
|libpangocairo-1.0-0|1.52.1+ds-1build1|
|libpangoft2-1.0-0|1.52.1+ds-1build1|
|libpangoft2-1.0-0|1.52.1+ds-1build1|
|libpangomm-1.4-1v5|2.46.4-1build3|
|libpangomm-2.48-1t64|2.52.0-1build1|
|libpangoxft-1.0-0|1.52.1+ds-1build1|
|libpaper1|1.1.29build1|
|libpaper-utils|1.1.29build1|
|libparted2t64|3.6-4build1|
|libpcap0.8t64|1.10.4-4.1ubuntu3|
|libpcap0.8t64|1.10.4-4.1ubuntu3|
|libpcaudio0|1.2-2build3|
|libpci3|1:3.10.0-2build1|
|libpciaccess0|0.17-3ubuntu0.24.04.2|
|libpciaccess0|0.17-3ubuntu0.24.04.2|
|libpciaccess-dev|0.17-3ubuntu0.24.04.2|
|libpcre2-16-0|10.42-4ubuntu2.1|
|libpcre2-32-0|10.42-4ubuntu2.1|
|libpcre2-8-0|10.42-4ubuntu2.1|
|libpcre2-8-0|10.42-4ubuntu2.1|
|libpcsclite1|2.0.3-1build1|
|libpcsclite1|2.0.3-1build1|
|libpeas-1.0-0|1.36.0-3build4|
|libpeas-common|1.36.0-3build4|
|libperl5.38t64|5.38.2-3.2ubuntu0.2|
|libperlio-gzip-perl|0.20-1build4|
|libpgm-5.3-0t64|5.3.128~dfsg-2.1build1|
|libphonenumber8|8.12.57+ds-4.2build3|
|libpipeline1|1.5.7-2|
|libpipewire-0.3-0t64|1.2.4-1ppa1|
|libpipewire-0.3-dev|1.2.4-1ppa1|
|libpipewire-0.3-modules|1.2.4-1ppa1|
|libpixman-1-0|0.42.2-1build1|
|libpixman-1-0|0.42.2-1build1|
|libpixman-1-dev|0.42.2-1build1|
|libpkcs11-helper1t64|1.29.0-2.1build2|
|libpkgconf3|1.8.1-2build1|
|libplacebo338|6.338.2-2build1|
|libplist-2.0-4|2.3.0-1~exp2build2|
|libplymouth5|24.004.60-1ubuntu7.1|
|libpng16-16t64|1.6.43-5ubuntu0.6|
|libpng16-16t64|1.6.43-5ubuntu0.6|
|libpocketsphinx3|0.8.0+real5prealpha+1-15ubuntu5|
|libpolkit-agent-1-0|124-2ubuntu1.24.04.3|
|libpolkit-gobject-1-0|124-2ubuntu1.24.04.3|
|libpoppler134|24.02.0-1ubuntu9.9|
|libpoppler-cpp0t64|24.02.0-1ubuntu9.9|
|libpoppler-glib8t64|24.02.0-1ubuntu9.9|
|libpopt0|1.19+dfsg-1build1|
|libportaudio2|19.6.0-1.2build3|
|libpostproc57|7:6.1.1-3ubuntu5|
|libppd2|2:2.0.0-0ubuntu4.1|
|libppd2-common|2:2.0.0-0ubuntu4.1|
|libproc2-0|2:4.0.4-4ubuntu3.2|
|libproc2-dev|2:4.0.4-4ubuntu3.2|
|libprotobuf32t64|3.21.12-8.2ubuntu0.3|
|libprotobuf-c1|1.4.1-1ubuntu4|
|libproxy1-plugin-gsettings|0.5.4-4build1|
|libproxy1-plugin-networkmanager|0.5.4-4build1|
|libproxy1v5|0.5.4-4build1|
|libproxy1v5|0.5.4-4build1|
|libpsl5t64|0.21.2-1.1build1|
|libpsl5t64|0.21.2-1.1build1|
|libpugixml1v5|1.14-0.1build1|
|libpulse0|1:16.1+dfsg1-2ubuntu10.1|
|libpulse0|1:16.1+dfsg1-2ubuntu10.1|
|libpulse-dev|1:16.1+dfsg1-2ubuntu10.1|
|libpulse-mainloop-glib0|1:16.1+dfsg1-2ubuntu10.1|
|libpwquality1|1.4.5-3build1|
|libpwquality-common|1.4.5-3build1|
|libpython3.12-minimal|3.12.3-1ubuntu0.13|
|libpython3.12-stdlib|3.12.3-1ubuntu0.13|
|libpython3.12t64|3.12.3-1ubuntu0.13|
|libpython3-stdlib|3.12.3-0ubuntu2.1|
|libqmi-glib5|1.35.2-0ubuntu2|
|libqmi-proxy|1.35.2-0ubuntu2|
|libqpdf29t64|11.9.0-1.1ubuntu0.1|
|libqrtr-glib0|1.2.2-1ubuntu4|
|libqt5core5t64|5.15.13+dfsg-1ubuntu1|
|libqt5dbus5t64|5.15.13+dfsg-1ubuntu1|
|libqt5gui5t64|5.15.13+dfsg-1ubuntu1|
|libqt5network5t64|5.15.13+dfsg-1ubuntu1|
|libqt5svg5|5.15.13-1|
|libqt5widgets5t64|5.15.13+dfsg-1ubuntu1|
|libquadmath0|16-20260315-1ubuntu1~24~ppa1|
|librabbitmq4|0.11.0-1build2|
|libraptor2-0|2.0.16-3ubuntu0.1|
|libraqm0|0.10.1-1build1|
|librasqal3t64|0.9.33-2.1build1|
|librav1e0|0.7.1-2|
|libraw1394-11|2.1.2-2build3|
|libraw1394-11|2.1.2-2build3|
|libraw23t64|0.21.2-2.1ubuntu0.24.04.1|
|librda0|0.0.5-1.1build2|
|librda-common|0.0.5-1.1build2|
|librdf0t64|1.0.17-3.1ubuntu3|
|libre2-dev|20230301-3build1|
|libreadline8t64|8.2-4build1|
|libreiserfscore0t64|1:3.6.27-7.1build1|
|libreoffice-base-core|4:24.2.7-0ubuntu0.24.04.5|
|libreoffice-calc|4:24.2.7-0ubuntu0.24.04.5|
|libreoffice-common|4:24.2.7-0ubuntu0.24.04.5|
|libreoffice-core|4:24.2.7-0ubuntu0.24.04.5|
|libreoffice-draw|4:24.2.7-0ubuntu0.24.04.5|
|libreoffice-gnome|4:24.2.7-0ubuntu0.24.04.5|
|libreoffice-gtk3|4:24.2.7-0ubuntu0.24.04.5|
|libreoffice-help-common|4:24.2.7-0ubuntu0.24.04.5|
|libreoffice-help-en-us|4:24.2.7-0ubuntu0.24.04.5|
|libreoffice-impress|4:24.2.7-0ubuntu0.24.04.5|
|libreoffice-style-colibre|4:24.2.7-0ubuntu0.24.04.5|
|libreoffice-uiconfig-calc|4:24.2.7-0ubuntu0.24.04.5|
|libreoffice-uiconfig-common|4:24.2.7-0ubuntu0.24.04.5|
|libreoffice-uiconfig-draw|4:24.2.7-0ubuntu0.24.04.5|
|libreoffice-uiconfig-impress|4:24.2.7-0ubuntu0.24.04.5|
|libreoffice-uiconfig-writer|4:24.2.7-0ubuntu0.24.04.5|
|libreoffice-writer|4:24.2.7-0ubuntu0.24.04.5|
|librest-1.0-0|0.9.1-6build3|
|librevenge-0.0-0|0.0.5-3build1|
|librhythmbox-core10|3.4.7-2ubuntu6|
|librist4|0.2.10+dfsg-2|
|libroc0.3|0.3.0+dfsg-7ubuntu2|
|librsvg2-2|2.58.0+dfsg-1build1|
|librsvg2-2|2.58.0+dfsg-1build1|
|librsvg2-bin|2.58.0+dfsg-1build1|
|librsvg2-common|2.58.0+dfsg-1build1|
|librsvg2-common|2.58.0+dfsg-1build1|
|librtmp1|2.4+20151223.gitfa8646d.1-2build7|
|librtmp1|2.4+20151223.gitfa8646d.1-2build7|
|librubberband2|3.3.0+dfsg-2build1|
|libsamplerate0|0.2.2-4build1|
|libsamplerate0|0.2.2-4build1|
|libsane1|1.2.1-7build4|
|libsane-common|1.2.1-7build4|
|libsane-hpaio|3.23.12+dfsg0-0ubuntu5|
|libsasl2-2|2.1.28+dfsg1-5ubuntu3.1|
|libsasl2-2|2.1.28+dfsg1-5ubuntu3.1|
|libsasl2-modules|2.1.28+dfsg1-5ubuntu3.1|
|libsasl2-modules|2.1.28+dfsg1-5ubuntu3.1|
|libsasl2-modules-db|2.1.28+dfsg1-5ubuntu3.1|
|libsasl2-modules-db|2.1.28+dfsg1-5ubuntu3.1|
|libsbc1|2.0-1build1|
|libsdbus-c++-dev|2.1.0-1ppa2|
|libsdl2-2.0-0|2.30.0+dfsg-1ubuntu3.1|
|libsdl2-2.0-0|2.30.0+dfsg-1ubuntu3.1|
|libsdl2-dev|2.30.0+dfsg-1ubuntu3.1|
|libseat-dev|0.8.0-1|
|libseccomp2|2.5.5-1ubuntu3.1|
|libsecret-1-0|0.21.4-1build3|
|libsecret-1-dev|0.21.4-1build3|
|libsecret-common|0.21.4-1build3|
|libselinux1|3.5-2ubuntu2.1|
|libselinux1|3.5-2ubuntu2.1|
|libsemanage2|3.5-1build5|
|libsemanage-common|3.5-1build5|
|libsensors5|1:3.6.0-9build1|
|libsensors5|1:3.6.0-9build1|
|libsensors-config|1:3.6.0-9build1|
|libsepol2|3.5-2build1|
|libserd-0-0|0.32.2-1|
|libsframe1|2.42-4ubuntu2.10|
|libsgutils2-1.46-2|1.46-3ubuntu4|
|libsharpyuv0|1.3.2-0.4build3|
|libsharpyuv0|1.3.2-0.4build3|
|libshine3|3.1.1-2build1|
|libshout3|2.4.6-1build2|
|libshout3|2.4.6-1build2|
|libsigc++-2.0-0v5|2.12.1-2|
|libsigc++-3.0-0|3.6.0-2|
|libsixel1|1.10.3-3build1|
|libslang2|2.3.3-3build2|
|libslang2|2.3.3-3build2|
|libsm6|2:1.2.3-1build3|
|libsmartcols1|2.39.3-9ubuntu6.5|
|libsmbclient0|2:4.19.5+dfsg-4ubuntu9.6|
|libsm-dev|2:1.2.3-1build3|
|libsnapd-glib-2-1|1.64-0ubuntu5|
|libsnappy1v5|1.1.10-1build1|
|libsndfile1|1.2.2-1ubuntu5.24.04.1|
|libsndfile1|1.2.2-1ubuntu5.24.04.1|
|libsndio7.0|1.9.0-0.3build3|
|libsnmp40t64|5.9.4+dfsg-1.1ubuntu3.2|
|libsnmp-base|5.9.4+dfsg-1.1ubuntu3.2|
|libsodium23|1.0.18-1ubuntu0.24.04.1|
|libsonic0|0.2.0-13build1|
|libsord-0-0|0.16.16-2build1|
|libsoup-2.4-1|2.74.3-6ubuntu1.6|
|libsoup2.4-common|2.74.3-6ubuntu1.6|
|libsoup-3.0-0|3.4.4-5ubuntu0.7|
|libsoup-3.0-0|3.4.4-5ubuntu0.7|
|libsoup-3.0-common|3.4.4-5ubuntu0.7|
|libsoup-gnome-2.4-1|2.74.3-6ubuntu1.6|
|libsource-highlight4t64|3.1.9-4.3build1|
|libsource-highlight-common|3.1.9-4.3build1|
|libsoxr0|0.1.3-4build3|
|libspa-0.2-bluetooth|1.2.4-1ppa1|
|libspa-0.2-modules|1.2.4-1ppa1|
|libspectre1|0.2.12-1build2|
|libspeechd2|0.12.0~rc2-2build3|
|libspeex1|1.2.1-2ubuntu2.24.04.1|
|libspeex1|1.2.1-2ubuntu2.24.04.1|
|libspeexdsp1|1.2.1-1ubuntu3|
|libsphinxbase3t64|0.8+5prealpha+1-17build2|
|libsqlite3-0|3.45.1-1ubuntu2.5|
|libsqlite3-0|3.45.1-1ubuntu2.5|
|libsratom-0-0|0.6.16-1build1|
|libsrt1.5-gnutls|1.5.3-1build2|
|libss2|1.47.0-2.4~exp1ubuntu4.1|
|libssh-4|0.10.6-2ubuntu0.4|
|libssh-4|0.10.6-2ubuntu0.4|
|libssh-gcrypt-4|0.10.6-2ubuntu0.4|
|libssl3t64|3.0.13-0ubuntu3.11|
|libssl3t64|3.0.13-0ubuntu3.11|
|libstartup-notification0|0.12-6build3|
|libstartup-notification0-dev|0.12-6build3|
|libstdc++-10-dev|10.5.0-4ubuntu2.1|
|libstdc++-12-dev|12.4.0-2ubuntu1~24.04.1|
|libstdc++-13-dev|13.3.0-6ubuntu2~24.04.1|
|libstdc++-14-dev|14.3.0-12ubuntu1~24~ppa1|
|libstdc++6|16-20260315-1ubuntu1~24~ppa1|
|libstdc++6|16-20260315-1ubuntu1~24~ppa1|
|libstemmer0d|2.2.0-4build1|
|libsuitesparseconfig7|1:7.6.1+dfsg-1build1|
|libsvtav1enc1d1|1.7.0+dfsg-2build1|
|libswresample4|7:6.1.1-3ubuntu5|
|libswscale7|7:6.1.1-3ubuntu5|
|libsystemd0|255.4-1ubuntu8.16|
|libsystemd0|255.4-1ubuntu8.16|
|libsystemd-dev|255.4-1ubuntu8.16|
|libsystemd-shared|255.4-1ubuntu8.16|
|libtag1v5|1.13.1-1build1|
|libtag1v5|1.13.1-1build1|
|libtag1v5-vanilla|1.13.1-1build1|
|libtag1v5-vanilla|1.13.1-1build1|
|libtalloc2|2.4.2-1build2|
|libtasn1-6|4.19.0-3ubuntu0.24.04.2|
|libtasn1-6|4.19.0-3ubuntu0.24.04.2|
|libtbb12|2021.11.0-2ubuntu2|
|libtbbmalloc2|2021.11.0-2ubuntu2|
|libtcl8.6|8.6.14+dfsg-1build1|
|libtcmalloc-minimal4t64|2.15-3build1|
|libtdb1|1.4.10-1build1|
|libteamdctl0|1.31-1build3|
|libtevent0t64|0.16.1-2build1|
|libtext-charwidth-perl|0.04-11build3|
|libtext-iconv-perl|1.7-8build3|
|libtext-wrapi18n-perl|0.06-10|
|libthai0|0.1.29-2build1|
|libthai0|0.1.29-2build1|
|libthai-data|0.1.29-2build1|
|libtheora0|1.1.1+dfsg.1-16.1build3|
|libtheora0|1.1.1+dfsg.1-16.1build3|
|libtie-ixhash-perl|1.23-4|
|libtiff6|4.5.1+git230720-4ubuntu2.5|
|libtiff6|4.5.1+git230720-4ubuntu2.5|
|libtiff-dev|4.5.1+git230720-4ubuntu2.5|
|libtimedate-perl|2.3300-2|
|libtimezonemap1|0.4.6-6ubuntu1|
|libtimezonemap-data|0.4.6-6ubuntu1|
|libtinfo6|6.4+20240113-1ubuntu2|
|libtinfo6|6.4+20240113-1ubuntu2|
|libtirpc3t64|1.3.4+ds-1.1build1|
|libtirpc-common|1.3.4+ds-1.1build1|
|libtomlplusplus3t64|3.4.0+ds-0.2build1|
|libtomlplusplus-dev|3.4.0+ds-0.2build1|
|libtool|2.4.7-7build1|
|libtotem-plparser18|3.26.6-1build5|
|libtotem-plparser-common|3.26.6-1build5|
|libtry-tiny-perl|0.31-2|
|libtsan2|16-20260315-1ubuntu1~24~ppa1|
|libtspi1|0.3.15-0.3build2|
|libtss2-esys-3.0.2-0t64|4.0.1-7.1ubuntu5.1|
|libtss2-mu-4.0.1-0t64|4.0.1-7.1ubuntu5.1|
|libtss2-sys1t64|4.0.1-7.1ubuntu5.1|
|libtss2-tcti-cmd0t64|4.0.1-7.1ubuntu5.1|
|libtss2-tcti-device0t64|4.0.1-7.1ubuntu5.1|
|libtss2-tcti-mssim0t64|4.0.1-7.1ubuntu5.1|
|libtss2-tcti-swtpm0t64|4.0.1-7.1ubuntu5.1|
|libtwolame0|0.4.0-2build3|
|libtwolame0|0.4.0-2build3|
|libubsan1|16-20260315-1ubuntu1~24~ppa1|
|libuchardet0|0.0.8-1build1|
|libudev1|255.4-1ubuntu8.16|
|libudev1|255.4-1ubuntu8.16|
|libudev-dev|255.4-1ubuntu8.16|
|libudfread0|1.1.2-1build1|
|libudisks2-0|2.10.1-6ubuntu1.3|
|libunibreak5|5.1-2build1|
|libunistring5|1.1-2build1.1|
|libunistring5|1.1-2build1.1|
|libuno-cppu3t64|4:24.2.7-0ubuntu0.24.04.5|
|libuno-cppuhelpergcc3-3t64|4:24.2.7-0ubuntu0.24.04.5|
|libuno-purpenvhelpergcc3-3t64|4:24.2.7-0ubuntu0.24.04.5|
|libuno-sal3t64|4:24.2.7-0ubuntu0.24.04.5|
|libuno-salhelpergcc3-3t64|4:24.2.7-0ubuntu0.24.04.5|
|libunwind8|1.6.2-3build1.1|
|libunwind8|1.6.2-3build1.1|
|libupower-glib3|1.90.3-1|
|liburcu8t64|0.14.0-3.1build1|
|liburing2|2.5-1build1|
|liburi-perl|5.27-1|
|libusb-1.0-0|2:1.0.27-1|
|libusb-1.0-0|2:1.0.27-1|
|libusb-1.0-0-dev|2:1.0.27-1|
|libusbmuxd6|2.0.2-4build3|
|libuuid1|2.39.3-9ubuntu6.5|
|libuutil3linux|2.2.2-0ubuntu9.4|
|libuv1t64|1.48.0-1.1build1|
|libv4l-0t64|1.26.1-4build3|
|libv4l-0t64|1.26.1-4build3|
|libv4lconvert0t64|1.26.1-4build3|
|libv4lconvert0t64|1.26.1-4build3|
|libva2|2.23.0-1~24.04~ppa5|
|libva2|2.23.0-1~24.04~ppa5|
|libva-dev|2.23.0-1~24.04~ppa5|
|libva-drm2|2.23.0-1~24.04~ppa5|
|libva-drm2|2.23.0-1~24.04~ppa5|
|libva-wayland2|2.23.0-1~24.04~ppa5|
|libva-x11-2|2.23.0-1~24.04~ppa5|
|libva-x11-2|2.23.0-1~24.04~ppa5|
|libvdpau1|1.5-2build1|
|libvdpau1|1.5-2build1|
|libvdpau-dev|1.5-2build1|
|libvidstab1.1|1.1.0-2build1|
|libvisio-0.1-1|0.1.7-1build9|
|libvisual-0.4-0|0.4.2-2build1|
|libvisual-0.4-0|0.4.2-2build1|
|libvisual-0.4-plugins|1:0.4.2+dfsg-2build3|
|libvkfft-dev|1.3.4+ds2-1|
|libvolume-key1|0.3.12-7build2|
|libvorbis0a|1.3.7-1build3|
|libvorbis0a|1.3.7-1build3|
|libvorbisenc2|1.3.7-1build3|
|libvorbisenc2|1.3.7-1build3|
|libvorbisfile3|1.3.7-1build3|
|libvpl2|1:2.16.0-1~24.04~ppa1|
|libvpl-tools|1.5.0-1~24.04~ppa1|
|libvpx9|1.14.0-1ubuntu2.3|
|libvpx9|1.14.0-1ubuntu2.3|
|libvte-2.91-0|0.76.0-1ubuntu0.1|
|libvte-2.91-common|0.76.0-1ubuntu0.1|
|libvulkan1|1.3.275.0-1build1|
|libvulkan1|1.3.275.0-1build1|
|libvulkan-dev|1.3.275.0-1build1|
|libvulkan-volk-dev|1.3.275.0-1|
|libwacom9|2.10.0-2|
|libwacom-bin|2.10.0-2|
|libwacom-common|2.10.0-2|
|libwacom-dev|2.10.0-2|
|libwavpack1|5.6.0-1build1|
|libwavpack1|5.6.0-1build1|
|libwayland-client0|1.22.0-2.1build1|
|libwayland-client0|1.22.0-2.1build1|
|libwayland-client++1|1.0.0-5ubuntu2|
|libwayland-cursor0|1.22.0-2.1build1|
|libwayland-cursor0|1.22.0-2.1build1|
|libwayland-dev|1.22.0-2.1build1|
|libwayland-egl1|1.22.0-2.1build1|
|libwayland-egl1|1.22.0-2.1build1|
|libwayland-server0|1.22.0-2.1build1|
|libwayland-server++1|1.0.0-5ubuntu2|
|libwbclient0|2:4.19.5+dfsg-4ubuntu9.6|
|libwebkit2gtk-4.1-0|2.52.3-0ubuntu0.24.04.1|
|libwebp7|1.3.2-0.4build3|
|libwebp7|1.3.2-0.4build3|
|libwebpdemux2|1.3.2-0.4build3|
|libwebpmux3|1.3.2-0.4build3|
|libwebrtc-audio-processing1|0.3.1-0ubuntu6|
|libwireplumber-0.4-0|0.4.17-1ubuntu4.1|
|libwlroots-dev|0.17.1-2.1build1|
|libwmf-0.2-7|0.2.13-1.1build3|
|libwmf-0.2-7-gtk|0.2.13-1.1build3|
|libwmf0.2-7-gtk|0.2.13-1.1build3|
|libwmflite-0.2-7|0.2.13-1.1build3|
|libwnck-3-0|43.0-3build4|
|libwnck-3-common|43.0-3build4|
|libwoff1|1.0.2-2build1|
|libwpd-0.10-10|0.10.3-2build2|
|libwpg-0.3-3|0.3.4-3build1|
|libwps-0.4-4|0.4.14-2build1|
|libwrap0|7.6.q-33|
|libwww-perl|6.76-1ubuntu0.1|
|libwww-robotrules-perl|6.02-1|
|libx11-6|2:1.8.7-1build1|
|libx11-6|2:1.8.7-1build1|
|libx11-data|2:1.8.7-1build1|
|libx11-dev|2:1.8.7-1build1|
|libx11-protocol-perl|0.56-9|
|libx11-xcb1|2:1.8.7-1build1|
|libx11-xcb1|2:1.8.7-1build1|
|libx11-xcb-dev|2:1.8.7-1build1|
|libx264-164|2:0.164.3108+git31e19f9-1|
|libx265-199|3.5-2build1|
|libxapian30|1.4.22-1build1|
|libxapp1|3.2.2+zena|
|libxatracker2|25.1.7-1ubuntu2~24.04.1|
|libxau6|1:1.0.9-1build6|
|libxau6|1:1.0.9-1build6|
|libxau-dev|1:1.0.9-1build6|
|libxaw7|2:1.0.14-1build2|
|libxcb1|1.15-1ubuntu2|
|libxcb1|1.15-1ubuntu2|
|libxcb-composite0-dev|1.15-1ubuntu2|
|libxcb-cursor-dev|0.1.4-1build1|
|libxcb-damage0|1.15-1ubuntu2|
|libxcb-dri2-0|1.15-1ubuntu2|
|libxcb-dri3-0|1.15-1ubuntu2|
|libxcb-dri3-0|1.15-1ubuntu2|
|libxcb-dri3-dev|1.15-1ubuntu2|
|libxcb-ewmh2|0.4.1-1.1build3|
|libxcb-ewmh-dev|0.4.1-1.1build3|
|libxcb-glx0|1.15-1ubuntu2|
|libxcb-glx0|1.15-1ubuntu2|
|libxcb-icccm4|0.4.1-1.1build3|
|libxcb-icccm4-dev|0.4.1-1.1build3|
|libxcb-image0|0.4.0-2build1|
|libxcb-keysyms1|0.4.0-1build4|
|libxcb-keysyms1-dev|0.4.0-1build4|
|libxcb-present0|1.15-1ubuntu2|
|libxcb-present0|1.15-1ubuntu2|
|libxcb-present-dev|1.15-1ubuntu2|
|libxcb-randr0|1.15-1ubuntu2|
|libxcb-randr0|1.15-1ubuntu2|
|libxcb-randr0-dev|1.15-1ubuntu2|
|libxcb-render0|1.15-1ubuntu2|
|libxcb-render0|1.15-1ubuntu2|
|libxcb-render-util0|0.3.9-1build4|
|libxcb-render-util0-dev|0.3.9-1build4|
|libxcb-res0|1.15-1ubuntu2|
|libxcb-res0-dev|1.15-1ubuntu2|
|libxcb-shape0|1.15-1ubuntu2|
|libxcb-shm0|1.15-1ubuntu2|
|libxcb-shm0|1.15-1ubuntu2|
|libxcb-sync1|1.15-1ubuntu2|
|libxcb-sync1|1.15-1ubuntu2|
|libxcb-util1|0.4.0-1build3|
|libxcb-util-dev|0.4.0-1build3|
|libxcb-xfixes0|1.15-1ubuntu2|
|libxcb-xfixes0|1.15-1ubuntu2|
|libxcb-xinerama0|1.15-1ubuntu2|
|libxcb-xinerama0-dev|1.15-1ubuntu2|
|libxcb-xinput0|1.15-1ubuntu2|
|libxcb-xinput-dev|1.15-1ubuntu2|
|libxcb-xkb1|1.15-1ubuntu2|
|libxcb-xkb1|1.15-1ubuntu2|
|libxcb-xv0|1.15-1ubuntu2|
|libxcomposite1|1:0.4.5-1build3|
|libxcomposite1|1:0.4.5-1build3|
|libxcomposite-dev|1:0.4.5-1build3|
|libxcursor1|1:1.2.1-1build1|
|libxcursor1|1:1.2.1-1build1|
|libxcursor-dev|1:1.2.1-1build1|
|libxcvt0|0.1.2-1build1|
|libxdamage1|1:1.1.6-1build1|
|libxdamage1|1:1.1.6-1build1|
|libxdamage-dev|1:1.1.6-1build1|
|libxdmcp6|1:1.1.3-0ubuntu6|
|libxdmcp6|1:1.1.3-0ubuntu6|
|libxdo3|1:3.20160805.1-5build1|
|libxext6|2:1.3.4-1build2|
|libxext6|2:1.3.4-1build2|
|libxext-dev|2:1.3.4-1build2|
|libxfixes3|1:6.0.0-2build1|
|libxfixes3|1:6.0.0-2build1|
|libxfixes-dev|1:6.0.0-2build1|
|libxfont2|1:2.0.6-1build1|
|libxft2|2.3.6-1build1|
|libxi6|2:1.8.1-1build1|
|libxi6|2:1.8.1-1build1|
|libxi-dev|2:1.8.1-1build1|
|libxinerama1|2:1.1.4-3build1|
|libxinerama1|2:1.1.4-3build1|
|libxinerama-dev|2:1.1.4-3build1|
|libxkbcommon0|1.12.3-1|
|libxkbcommon0|1.12.3-1|
|libxkbcommon-dev|1.12.3-1|
|libxkbcommon-x11-0|1.12.3-1|
|libxkbcommon-x11-0|1.12.3-1|
|libxkbcommon-x11-dev|1.12.3-1|
|libxkbfile1|1:1.1.0-1build4|
|libxkbfile-dev|1:1.1.0-1build4|
|libxkbregistry0|1.12.3-1|
|libxkbregistry0|1.12.3-1|
|libxkbregistry-dev|1.12.3-1|
|libxklavier16|5.4-5build2|
|libxml2|2.9.14+dfsg-1.3ubuntu3.7|
|libxml2|2.9.14+dfsg-1.3ubuntu3.7|
|libxml2-dev|2.9.14+dfsg-1.3ubuntu3.7|
|libxmlb2|0.3.18-1|
|libxml-namespacesupport-perl|1.12-2|
|libxml-parser-perl|2.47-1ubuntu0.24.04.1|
|libxml-sax-base-perl|1.09-3|
|libxml-sax-expat-perl|0.51-2|
|libxml-sax-perl|1.02+dfsg-3|
|libxmlsec1t64|1.2.39-5build2|
|libxmlsec1t64-nss|1.2.39-5build2|
|libxml-twig-perl|1:3.52-2|
|libxml-xpathengine-perl|0.14-2|
|libxmu6|2:1.1.3-3build2|
|libxmuu1|2:1.1.3-3build2|
|libxpm4|1:3.5.17-1build2|
|libxpm4|1:3.5.17-1build2|
|libxpresent1|1.0.0-2build2|
|libxpum-dev|1.3.6-1~24.04~ppa1|
|libxrandr2|2:1.5.2-2build1|
|libxrandr2|2:1.5.2-2build1|
|libxrandr-dev|2:1.5.2-2build1|
|libxreaderdocument3|4.6.5+wilma|
|libxreaderview3|4.6.5+wilma|
|libxrender1|1:0.9.10-1.1build1|
|libxrender1|1:0.9.10-1.1build1|
|libxrender-dev|1:0.9.10-1.1build1|
|libxres1|2:1.2.1-1build1|
|libxshmfence1|1.3-1build5|
|libxshmfence1|1.3-1build5|
|libxslt1.1|1.1.39-0exp1ubuntu0.24.04.3|
|libxss1|1:1.2.3-1build3|
|libxss1|1:1.2.3-1build3|
|libxt6t64|1:1.2.1-1.2build1|
|libxtables12|1.8.10-3ubuntu2|
|libxt-dev|1:1.2.1-1.2build1|
|libxtst6|2:1.2.3-1.1build1|
|libxtst6|2:1.2.3-1.1build1|
|libxv1|2:1.0.11-1.1build1|
|libxv1|2:1.0.11-1.1build1|
|libxvidcore4|2:1.3.7-1build1|
|libxvmc1|2:1.0.12-2build3|
|libxxf86dga1|2:1.1.5-1build1|
|libxxf86vm1|1:1.1.4-1build4|
|libxxf86vm1|1:1.1.4-1build4|
|libxxhash0|0.8.2-2build1|
|libxxhash-dev|0.8.2-2build1|
|libyajl2|2.1.0-5build1|
|libyaml-0-2|0.2.5-1build1|
|libyelp0|42.2-1ubuntu0.24.04.1|
|libze1|1.28.2-1~24.04~ppa1|
|libze-intel-gpu1|26.18.38308.1-1~24.04~ppa1|
|libze-intel-gpu-raytracing|1.2.2-1~24.04~ppa1|
|libzfs4linux|2.2.2-0ubuntu9.4|
|libzimg2|3.0.5+ds1-1build1|
|libzip4t64|1.7.3-1.1ubuntu2|
|libzix-0-0|0.4.2-2build1|
|libzmq5|4.3.5-1build2|
|libzopfli1|1.0.3-2|
|libzpool5linux|2.2.2-0ubuntu9.4|
|libzstd1|1.5.5+dfsg2-2build1.1|
|libzstd1|1.5.5+dfsg2-2build1.1|
|libzvbi0t64|0.2.42-2|
|libzvbi-common|0.2.42-2|
|lightdm|1.30.0-0ubuntu14|
|lightdm-settings|2.1.1|
|linux-base|4.5ubuntu9+24.04.2|
|linux-firmware|20240318.git3b128b60-0ubuntu2.27|
|linux-generic-7.0|7.0.0-14.14~24.04.3|
|linux-headers-6.18.18-061818|6.18.18-061818.202603131642|
|linux-headers-6.18.18-061818-generic|6.18.18-061818.202603131642|
|linux-headers-6.19.10-061910|6.19.10-061910.202603251147|
|linux-headers-6.19.10-061910-generic|6.19.10-061910.202603251147|
|linux-headers-7.0.0-14-generic|7.0.0-14.14~24.04.3|
|linux-headers-generic-7.0|7.0.0-14.14~24.04.3|
|linux-hwe-6.11-headers-6.11.0-17|6.11.0-17.17~24.04.2|
|linux-hwe-6.11-headers-6.11.0-19|6.11.0-19.19~24.04.1|
|linux-hwe-6.11-headers-6.11.0-21|6.11.0-21.21~24.04.1|
|linux-hwe-6.11-headers-6.11.0-24|6.11.0-24.24~24.04.1|
|linux-hwe-6.11-headers-6.11.0-26|6.11.0-26.26~24.04.1|
|linux-hwe-6.11-headers-6.11.0-28|6.11.0-28.28~24.04.1|
|linux-hwe-6.11-tools-6.11.0-17|6.11.0-17.17~24.04.2|
|linux-hwe-6.11-tools-6.11.0-19|6.11.0-19.19~24.04.1|
|linux-hwe-6.11-tools-6.11.0-21|6.11.0-21.21~24.04.1|
|linux-hwe-6.11-tools-6.11.0-24|6.11.0-24.24~24.04.1|
|linux-hwe-6.11-tools-6.11.0-26|6.11.0-26.26~24.04.1|
|linux-hwe-6.11-tools-6.11.0-28|6.11.0-28.28~24.04.1|
|linux-hwe-6.14-headers-6.14.0-24|6.14.0-24.24~24.04.3|
|linux-hwe-6.14-tools-6.14.0-24|6.14.0-24.24~24.04.3|
|linux-hwe-6.17-headers-6.17.0-14|6.17.0-14.14~24.04.1|
|linux-hwe-6.17-headers-6.17.0-19|6.17.0-19.19~24.04.2|
|linux-hwe-6.17-tools-6.17.0-14|6.17.0-14.14~24.04.1|
|linux-hwe-6.17-tools-6.17.0-19|6.17.0-19.19~24.04.2|
|linux-hwe-7.0-headers-7.0.0-14|7.0.0-14.14~24.04.3|
|linux-hwe-7.0-tools-7.0.0-14|7.0.0-14.14~24.04.3|
|linux-image-7.0.0-14-generic|7.0.0-14.14~24.04.3|
|linux-image-generic-7.0|7.0.0-14.14~24.04.3|
|linux-image-unsigned-6.18.18-061818-generic|6.18.18-061818.202603131642|
|linux-image-unsigned-6.19.10-061910-generic|6.19.10-061910.202603251147|
|linux-libc-dev|6.8.0-124.124|
|linuxmint-keyring|2022.06.21|
|linux-modules-6.18.18-061818-generic|6.18.18-061818.202603131642|
|linux-modules-6.19.10-061910-generic|6.19.10-061910.202603251147|
|linux-modules-7.0.0-14-generic|7.0.0-14.14~24.04.3|
|linux-sound-base|1.0.25+dfsg-0ubuntu7|
|linux-tools-6.11.0-17-generic|6.11.0-17.17~24.04.2|
|linux-tools-6.11.0-19-generic|6.11.0-19.19~24.04.1|
|linux-tools-6.11.0-21-generic|6.11.0-21.21~24.04.1|
|linux-tools-6.11.0-24-generic|6.11.0-24.24~24.04.1|
|linux-tools-6.11.0-26-generic|6.11.0-26.26~24.04.1|
|linux-tools-6.14.0-24-generic|6.14.0-24.24~24.04.3|
|linux-tools-7.0.0-14-generic|7.0.0-14.14~24.04.3|
|lldb|1:18.0-59~exp2|
|lm-sensors|1:3.6.0-9build1|
|localechooser-data|2.89ubuntu6|
|locales|2.39-0ubuntu8.7|
|login|1:4.13+dfsg1-4ubuntu3.2|
|logrotate|3.21.0-2build1|
|logsave|1.47.0-2.4~exp1ubuntu4.1|
|loupe|46.2-0ubuntu1|
|lp-solve|5.5.2.5-2ubuntu0.1|
|lsb-release|12.0-2|
|lsfg-vk|1.0.0-1|
|lshw|02.19.git.2021.06.19.996aaad9c7-2ubuntu0.24.04.1|
|lsof|4.95.0-1build3|
|lto-disabled-list|47|
|ltrace|0.7.3-6.4ubuntu3|
|lua5.4|5.4.6-3build2|
|lutris|0.5.20|
|lvm2|2.03.16-3ubuntu3.2|
|mailcap|3.70+nmu1ubuntu1|
|mainline|1.4.13-0~202505020443~ubuntu24.04.1|
|make|4.3-4.1build2|
|man-db|2.12.0-4build2|
|mangohud|0.6.9.1-2build1|
|manpages|6.7-2|
|mate-desktop|1.26.2-1.1mint1+zara|
|mate-desktop-common|1.26.2-1.1mint1+zara|
|mate-menus|1.26.1+mint1|
|mate-panel|1.27.1-2build3|
|mate-panel-common|1.27.1-2build3|
|mate-polkit|1.26.1+mint1|
|mate-polkit-common|1.26.1+mint1|
|mawk|1.3.4.20240123-1build1|
|mdadm|4.3-1ubuntu2.1|
|media-player-info|24-3|
|media-types|10.1.0|
|mesa-libgallium|26.1.2~kisak1~n|
|mesa-libgallium|26.1.2~kisak1~n|
|mesa-utils|9.0.0-2|
|mesa-utils-bin|9.0.0-2|
|mesa-vdpau-drivers|26.1.2~kisak1~n|
|mesa-vulkan-drivers|26.1.2~kisak1~n|
|mesa-vulkan-drivers|26.1.2~kisak1~n|
|meson|1.3.2-1ubuntu1|
|metacity|1:3.52.0-1build1|
|metacity-common|1:3.52.0-1build1|
|mgba-sdl|0.10.5|
|micro|2.0.13-1|
|mint-artwork|1.9.3|
|mint-backgrounds-wallpapers|1.0|
|mint-backgrounds-wilma|1.1|
|mintbackup|2.4.7|
|mint-common|2.5.1|
|mint-cursor-themes|1.0.2|
|mint-dev-tools|1.5.3|
|mintdrivers|1.8.8|
|mint-info-cinnamon|2025.11.11|
|mintinstall|8.4.0|
|mint-l-icons|1.8.0|
|mintlocale|1.6.6|
|mint-l-theme|2.0.6|
|mintmenu|6.2.3|
|mint-meta-cinnamon|2025.12.15+mint22.3|
|mint-meta-codecs|2025.12.15+mint22.3|
|mint-meta-core|2025.12.15+mint22.3|
|mint-mirrors|1.2.9|
|mintreport|1.6.1|
|mintsources|2.4.3|
|mintstick|1.6.6|
|mintsysadm|1.0.4|
|mintsystem|8.6.5|
|mint-themes|2.3.8|
|mint-translations|2026.01.08|
|mintupdate|7.1.4|
|mint-upgrade-info|1.3.0|
|mintwelcome|2.6.2|
|mint-x-icons|1.7.5|
|mint-y-icons|1.9.1|
|mitmproxy|8.1.1-2|
|mobile-broadband-provider-info|20230416-1|
|mockoon|9.1.0|
|modemmanager|1.23.4-0ubuntu2|
|mokutil|0.6.0-2build3|
|mono-complete|6.8.0.105+dfsg-3.6ubuntu2|
|mono-runtime|6.8.0.105+dfsg-3.6ubuntu2|
|mount|2.39.3-9ubuntu6.5|
|mousepad|0.6.1-1build2|
|mousetweaks|3.32.0-4build2|
|mpv|0.37.0-1ubuntu4|
|mpv-mpris|0.7.1-1build2|
|mscompress|0.4-10build1|
|mtools|4.0.43-1build1|
|mtr-tiny|0.95-1.1ubuntu0.1|
|muffin|6.6.3+zena|
|muffin-common|6.6.3+zena|
|mythes-en-us|1:24.2.1-1|
|nano|7.2-2ubuntu0.2|
|nasm|2.16.01-1build1|
|nautilus-data|1:46.4-0ubuntu0.2|
|ncurses-base|6.4+20240113-1ubuntu2|
|ncurses-bin|6.4+20240113-1ubuntu2|
|nemo|6.6.3+zena|
|nemo-data|6.6.3+zena|
|nemo-dbg|6.6.3+zena|
|nemo-emblems|6.6.0+zena|
|nemo-fileroller|6.6.0+zena|
|nemo-preview|6.6.0+zena|
|nemo-share|6.6.0+zena|
|neofetch|7.1.0-4|
|neovim|0.9.5-6ubuntu2|
|neowofetch|1.4.11-1|
|netbase|6.4|
|netcat-openbsd|1.226-1ubuntu2|
|netplan-generator|1.1.2-8ubuntu1~24.04.2|
|netplan.io|1.1.2-8ubuntu1~24.04.2|
|net-tools|2.10-0.1ubuntu4.4|
|networkd-dispatcher|2.2.4-1|
|network-manager|1.46.0-1ubuntu2.7|
|network-manager-config-connectivity-ubuntu|1.46.0-1ubuntu2.7|
|network-manager-gnome|1.34.0+mint3|
|network-manager-openvpn|1.10.2-4ubuntu0.2|
|network-manager-openvpn-gnome|1.10.2-4ubuntu0.2|
|network-manager-pptp|1.2.12-3build2|
|network-manager-pptp-gnome|1.2.12-3build2|
|nftables|1.0.9-1ubuntu0.1|
|ninja-build|1.11.1-2|
|nix-setup-systemd|2.18.1+dfsg-1ubuntu5|
|nm-tray|0.5.0-0ubuntu4|
|ntfs-3g|1:2022.10.3-1.2ubuntu3.1|
|nvtop|3.0.2-1|
|obs-studio|32.1.2-0obsproject1~noble|
|ocl-icd-libopencl1|2.3.2-1build1|
|ocl-icd-libopencl1|2.3.2-1build1|
|odt2txt|0.5-7build1|
|ohcount|4.0.0-4build2|
|onboard|1.4.1+mint4+wilma|
|onboard-common|1.4.1+mint4+wilma|
|openjdk-25-jdk|25.0.3+9-2~24.04.2|
|openprinting-ppds|20230202-1|
|openssh-client|1:9.6p1-3ubuntu13.16|
|openssl|3.0.13-0ubuntu3.11|
|openvpn|2.6.19-0ubuntu0.24.04.2|
|open-whispr|1.7.0|
|orca|46.1-1ubuntu1|
|p11-kit|0.25.3-4ubuntu2.1|
|p11-kit-modules|0.25.3-4ubuntu2.1|
|p7zip|16.02+transitional.1|
|p7zip-full|16.02+transitional.1|
|packagekit|1.2.8+mint3+wilma|
|packagekit-tools|1.2.8+mint3+wilma|
|packages-microsoft-prod|1.1-debian12|
|pamixer|1.6-1|
|pandoc|3.1.3+ds-2|
|pandora-launcher|5.0.1|
|papirus-icon-theme|20240201-1|
|parted|3.6-4build1|
|passwd|1:4.13+dfsg1-4ubuntu3.2|
|patch|2.7.6-7build3|
|patchutils|0.4.2-1build3|
|pavucontrol|5.0-2build3|
|pci.ids|0.0~2024.03.31-1ubuntu0.1|
|pciutils|1:3.10.0-2build1|
|pcmciautils|018-14build1|
|perl|5.38.2-3.2ubuntu0.2|
|perl-base|5.38.2-3.2ubuntu0.2|
|perl-modules-5.38|5.38.2-3.2ubuntu0.2|
|perl-openssl-defaults|7build3|
|pinentry-curses|1.2.1-3ubuntu5|
|pinentry-gnome3|1.2.1-3ubuntu5|
|piper|0.7-1|
|pipewire|1.2.4-1ppa1|
|pipewire-bin|1.2.4-1ppa1|
|pipewire-pulse|1.2.4-1ppa1|
|pipx|1.4.3-1|
|pix|3.4.10+zena|
|pix-data|3.4.10+zena|
|pix-dbg|3.4.10+zena|
|pkexec|124-2ubuntu1.24.04.3|
|pkgconf|1.8.1-2build1|
|pkgconf-bin|1.8.1-2build1|
|pkg-config|1.8.1-2build1|
|plasma-workspace-wayland|4:5.27.12-0ubuntu0.1|
|playerctl|2.4.1-2build2|
|plocate|1.1.19-2ubuntu2|
|plymouth|24.004.60-1ubuntu7.1|
|plymouth-label|24.004.60-1ubuntu7.1|
|plymouth-theme-spinner|24.004.60-1ubuntu7.1|
|plymouth-theme-ubuntu-text|24.004.60-1ubuntu7.1|
|pm-utils|1.4.1-21|
|policykit-1|124-2ubuntu1.24.04.3|
|policykit-1-gnome|0.105-7ubuntu5|
|policykit-desktop-privileges|0.22|
|polkitd|124-2ubuntu1.24.04.3|
|poppler-data|0.4.12-1|
|poppler-utils|24.02.0-1ubuntu9.9|
|powermgmt-base|1.37ubuntu0.1|
|power-profiles-daemon|0.21-1ubuntu1|
|ppp|2.4.9-1+1.1ubuntu4|
|pptp-linux|1.10.0-1build4|
|printer-driver-brlaser|6-3build2|
|printer-driver-c2esp|27-11ubuntu7|
|printer-driver-foo2zjs|20200505dfsg0-2ubuntu6|
|printer-driver-foo2zjs-common|20200505dfsg0-2ubuntu6|
|printer-driver-gutenprint|5.3.4.20220624T01008808d602-1build4|
|printer-driver-hpcups|3.23.12+dfsg0-0ubuntu5|
|printer-driver-m2300w|0.51-15build2|
|printer-driver-min12xxw|0.0.9-11build3|
|printer-driver-pnm2ppa|1.13+nondbs-0ubuntu10|
|printer-driver-postscript-hp|3.23.12+dfsg0-0ubuntu5|
|printer-driver-ptouch|1.7-1build2|
|printer-driver-pxljr|1.4+repack0-6build2|
|printer-driver-sag-gdi|0.1-8|
|printer-driver-splix|2.0.0+svn315-7fakesync1ubuntu1|
|procps|2:4.0.4-4ubuntu3.2|
|proguard-cli|6.2.2-3|
|psmisc|23.7-1build1|
|publicsuffix|20231001.0357-0.1|
|pulseaudio-utils|1:16.1+dfsg1-2ubuntu10.1|
|python3.12|3.12.3-1ubuntu0.13|
|python3.12-minimal|3.12.3-1ubuntu0.13|
|python3|3.12.3-0ubuntu2.1|
|python3-apport|2.28.1-0ubuntu3.8|
|python3-apt|2.7.7ubuntu5.2|
|python3-aptdaemon|1.1.1+bzr982-0ubuntu44mint1|
|python3-aptdaemon.gtk3widgets|1.1.1+bzr982-0ubuntu44mint1|
|python3-babel|2.10.3-3build1|
|python3-blinker|1.7.0-1|
|python3-bpfcc|0.29.1+ds-1ubuntu7|
|python3-brlapi|6.6-4ubuntu5|
|python3-brotli|1.1.0-2build2|
|python3-bs4|4.12.3-1|
|python3-cairo|1.25.1-2build2|
|python3-certifi|2023.11.17-1|
|python3-cffi-backend|1.16.0-2build1|
|python3-chardet|5.2.0+dfsg-1|
|python3-click|8.1.6-2|
|python3-colorama|0.4.6-4|
|python3-commandnotfound|23.04.0|
|python3-configobj|5.0.8-3|
|python3-cryptography|41.0.7-4ubuntu0.4|
|python3-cups|2.0.1-5build6|
|python3-cupshelpers|1.5.18-1ubuntu9|
|python3-dbus|1.3.2-5build3|
|python3-debian|0.1.49ubuntu2|
|python3-defer|1.0.6-2.1ubuntu1|
|python3-deprecation|2.1.0-3|
|python3-distro|1.9.0-1|
|python3-eyed3|0.9.7-1|
|python3-filelock|3.13.1-1|
|python3-filetype|1.2.0-1|
|python3-gdbm|3.12.3-0ubuntu1|
|python3-gi|3.48.2-1|
|python3-gi-cairo|3.48.2-1|
|python3-gnupg|0.5.2-1|
|python3-httplib2|0.20.4-3|
|python3-ibus-1.0|1.5.29-2|
|python3-icu|2.12-1build2|
|python3-idna|3.6-2ubuntu0.1|
|python3-ifaddr|0.2.0-1|
|python3-imdbpy|2021.04.18-3|
|python3-jwt|2.7.0-1ubuntu0.1|
|python3-launchpadlib|1.11.0-6|
|python3-lazr.restfulclient|0.14.6-1|
|python3-lazr.uri|1.0.6-3|
|python3-ldb|2:2.8.0+samba4.19.5+dfsg-4ubuntu9.6|
|python3-libevdev|0.5-3|
|python3-louis|3.29.0-1build1|
|python3-lxml|5.2.1-1|
|python3-magic|2:0.4.27-3|
|python3-mako|1.3.2-1ubuntu0.1|
|python3-markdown|3.5.2-1|
|python3-markdown-it|3.0.0-2|
|python3-markupsafe|2.1.5-1build2|
|python3-mdurl|0.1.2-1|
|python3-minimal|3.12.3-0ubuntu2.1|
|python3-mutagen|1.46.0-2|
|python3-nacl|1.5.0-4build1|
|python3-netaddr|0.8.0-2ubuntu1|
|python3-netifaces|0.11.0-2build3|
|python3-netplan|1.1.2-8ubuntu1~24.04.2|
|python3-oauthlib|3.2.2-1|
|python3-packaging|24.0-1|
|python3-pam|0.4.2-18build2|
|python3-parted|3.12.0-6build3|
|python3-pexpect|4.9-2|
|python3-pil|10.2.0-1ubuntu1.2|
|python3-pip|24.0+dfsg-1ubuntu1.3|
|python3-pkg-resources|68.1.2-2ubuntu1.2|
|python3-png|0.20231004.0-2|
|python3-polib|1.1.1-1|
|python3-problem-report|2.28.1-0ubuntu3.8|
|python3-psutil|5.9.8-2build2|
|python3-ptyprocess|0.7.0-5|
|python3-pyasyncore|1.0.2-2|
|python3-pyatspi|2.46.1-1|
|python3-pycryptodome|3.20.0+dfsg-1|
|python3-pycurl|7.45.3-1build2|
|python3-pyelftools|0.30-1|
|python3-pygments|2.17.2+dfsg-1|
|python3-pyinotify|0.9.6-2ubuntu1|
|python3-pyparsing|3.1.1-1|
|python3-pyquery|1.4.3-1|
|python3-pyudev|0.24.0-1|
|python3-qrcode|7.4.2-4|
|python3-repolib|2.2.1+mint1|
|python3-requests|2.31.0+dfsg-1ubuntu1.1|
|python3-requests-file|1.5.1-2|
|python3-rich|13.7.1-1|
|python3-samba|2:4.19.5+dfsg-4ubuntu9.6|
|python3-setproctitle|1.3.3-1build2|
|python3-six|1.16.0-4|
|python3-soupsieve|2.5-1|
|python3-speechd|0.12.0~rc2-2build3|
|python3-systemd|235-1build4|
|python3-talloc|2.4.2-1build2|
|python3-tdb|1.4.10-1build1|
|python3-tinycss2|1.2.1-2|
|python3-tldextract|3.1.2-2|
|python3-typing-extensions|4.10.0-1|
|python3-tz|2024.1-2|
|python3-unidecode|1.3.8-1|
|python3-uno|4:24.2.7-0ubuntu0.24.04.5|
|python3-urllib3|2.0.7-1ubuntu0.7|
|python3-venv|3.12.3-0ubuntu2.1|
|python3-wadllib|1.3.6-5|
|python3-webencodings|0.5.1-5|
|python3-websockets|10.4-1|
|python3-xapp|3.0.2|
|python3-xdg|0.28-2|
|python3-xkit|0.5.0ubuntu6|
|python3-xlib|0.33-2|
|python3-xlrd|2.0.1-2|
|python3-yaml|6.0.1-2build2|
|python-apt-common|2.7.7ubuntu5.2|
|python-babel-localedata|2.10.3-3build1|
|python-is-python3|3.11.4-1|
|python-nemo|6.6.0+zena|
|python-tinycss2-common|1.2.1-2|
|qalculate-gtk|4.9.0-1build3|
|qt5ct|1.5-1build11|
|qt5-gtk2-platformtheme|5.0.0+git23.g335dbec-6build5|
|qt5-gtk-platformtheme|5.15.13+dfsg-1ubuntu1|
|qt5-style-kvantum|1.0.10-1build3|
|qt5-style-plugin-cleanlooks|5.0.0+git23.g335dbec-6build5|
|qt5-style-plugin-motif|5.0.0+git23.g335dbec-6build5|
|qt5-style-plugin-plastique|5.0.0+git23.g335dbec-6build5|
|qt5-style-plugins|5.0.0+git23.g335dbec-6build5|
|qt6-base-dev|6.4.2+dfsg-21.1build5|
|qt6-base-dev-tools|6.4.2+dfsg-21.1build5|
|qt6ct|0.9-2build2|
|qt6-declarative-dev|6.4.2+dfsg-4build3|
|qt6-svg-dev|6.4.2-4ubuntu3|
|qt6-tools-dev-tools|6.4.2-3build3|
|qt6-wayland|6.4.2-5build3|
|qt6-wayland-dev|6.4.2-5build3|
|qttranslations5-l10n|5.15.13-1|
|r2modman|3.2.15|
|rdate|1:1.11-3ubuntu2|
|readline-common|8.2-4build1|
|reiserfsprogs|1:3.6.27-7.1build1|
|rename|2.02-1|
|rfkill|2.39.3-9ubuntu6.5|
|rhythmbox|3.4.7-2ubuntu6|
|rhythmbox-data|3.4.7-2ubuntu6|
|rhythmbox-plugins|3.4.7-2ubuntu6|
|rhythmbox-plugin-tray-icon|2020.05.21|
|ripgrep|14.1.0-1|
|rpcsvc-proto|1.4.2-0ubuntu7|
|rsync|3.2.7-1ubuntu1.5|
|rsyslog|8.2312.0-3ubuntu9.2|
|rtkit|0.13-5build1|
|samba-common|2:4.19.5+dfsg-4ubuntu9.6|
|samba-common-bin|2:4.19.5+dfsg-4ubuntu9.6|
|samba-libs|2:4.19.5+dfsg-4ubuntu9.6|
|sane-airscan|0.99.29-0ubuntu4|
|sane-utils|1.2.1-7build4|
|sbsigntool|0.9.4-3.1ubuntu7|
|scdoc|1.11.2-1|
|scons|4.5.2+dfsg-1|
|scrcpy|1.25-1ubuntu1|
|sddm|0.20.0-2ubuntu4.2|
|sddm-theme-breeze|4:5.27.12-0ubuntu0.1|
|seahorse|43.0-3build2|
|seatd|0.8.0-1|
|secureboot-db|1.9build1|
|sed|4.9-2ubuntu0.24.04.1|
|sensible-utils|0.0.22|
|session-migration|0.3.9build1|
|sgml-base|1.31|
|sgml-data|2.0.11+nmu1|
|shared-mime-info|2.4-4|
|simple-scan|46.0-0ubuntu2.1mint1+zena|
|slick-greeter|2.2.6+zena|
|slurp|1.5.0-1|
|smbclient|2:4.19.5+dfsg-4ubuntu9.6|
|software-properties-common|2.4.3|
|sound-theme-freedesktop|0.8-2ubuntu1|
|speech-dispatcher|0.12.0~rc2-2build3|
|speech-dispatcher-audio-plugins|0.12.0~rc2-2build3|
|speech-dispatcher-espeak-ng|0.12.0~rc2-2build3|
|squashfs-tools|1:4.6.1-1build1|
|ssl-cert|1.1.2ubuntu1|
|steam|1:1.0.0.85|
|steam-libs-amd64|1:1.0.0.85|
|steam-libs-i386|1:1.0.0.85|
|sticky|1.31|
|stow|2.3.1-1|
|strace|6.8-0ubuntu2|
|streamer|3.107-2build3|
|sudo|1.9.15p5-3ubuntu5.24.04.2|
|sunshine|2025.628.4510|
|sway-notification-center|0.9.0-1build2|
|switcheroo-control|2.6-2build2|
|synaptic|0.91.3build4|
|syslinux|3:6.04~git20190206.bf6db5b4+dfsg1-3ubuntu3|
|syslinux-common|3:6.04~git20190206.bf6db5b4+dfsg1-3ubuntu3|
|syslinux-utils|3:6.04~git20190206.bf6db5b4+dfsg1-3ubuntu3|
|system-config-printer|1.5.18-1ubuntu9|
|system-config-printer-common|1.5.18-1ubuntu9|
|system-config-printer-udev|1.5.18-1ubuntu9|
|systemd|255.4-1ubuntu8.16|
|systemd-coredump|255.4-1ubuntu8.16|
|systemd-dev|255.4-1ubuntu8.16|
|systemd-hwe-hwdb|255.1.7|
|systemd-resolved|255.4-1ubuntu8.16|
|systemd-sysv|255.4-1ubuntu8.16|
|systemd-timesyncd|255.4-1ubuntu8.16|
|system-tools-backends|2.10.2-3.1ubuntu4|
|sysvinit-utils|3.08-6ubuntu3|
|t1utils|1.41-4build3|
|tar|1.35+dfsg-3build1|
|tcl|8.6.14build1|
|tcl8.6|8.6.14+dfsg-1build1|
|tcpdump|4.99.4-3ubuntu4.24.04.1|
|telnet|0.17+2.5-3ubuntu4.2|
|thermald|2.5.6-2ubuntu0.24.04.5|
|thingy|1.2.4|
|thunderbird|1:140.11.1esr+linuxmint1+zena|
|thunderbird-locale-en|1:140.11.1esr+linuxmint1+zena|
|thunderbird-locale-en-us|1:140.11.1esr+linuxmint1+zena|
|time|1.9-0.2build1|
|timeshift|25.12.4+zena|
|tnftp|20230507-2build3|
|touchegg|2.0.16+victoria|
|tpm-udev|0.6ubuntu1|
|transmission-common|4.0.5-1build5mint1+wilma|
|transmission-gtk|4.0.5-1build5mint1+wilma|
|trash-cli|0.23.11.10-1|
|ttf-mscorefonts-installer|3.8.1ubuntu1|
|tzdata|2026a-0ubuntu0.24.04.1|
|tzdata-legacy|2026a-0ubuntu0.24.04.1|
|ubuntu-dbgsym-keyring|2023.11.28.1|
|ubuntu-drivers-common|1:0.9.7.6ubuntu3.7|
|ubuntu-kernel-accessories|1.539.2|
|ubuntu-keyring|2023.11.28.1|
|ubuntu-mono|24.04-0ubuntu1|
|ubuntu-system-adjustments|2025.12.16-zena|
|ucf|3.0043+nmu1|
|udev|255.4-1ubuntu8.16|
|udisks2|2.10.1-6ubuntu1.3|
|ueli|9.24.0|
|ufw|0.36.2-6|
|unityhub|3.15.2|
|uno-libs-private|4:24.2.7-0ubuntu0.24.04.5|
|unrar|1:7.0.7-1build1|
|untex|1:1.2-7build1|
|unzip|6.0-28ubuntu4.1|
|update-inetd|4.53|
|upower|1.90.3-1|
|ure|4:24.2.7-0ubuntu0.24.04.5|
|usb.ids|2024.03.18-1|
|usb-modeswitch|2.6.1-3ubuntu3|
|usb-modeswitch-data|20191128-6|
|usbmuxd|1.1.1-5~exp3ubuntu2.1|
|usbutils|1:017-3build1|
|user-setup|1.95ubuntu1|
|util-linux|2.39.3-9ubuntu6.5|
|uuid-runtime|2.39.3-9ubuntu6.5|
|vainfo|2.23.0-1~24.04~ppa4|
|vim-common|2:9.1.0016-1ubuntu7.15|
|vim-tiny|2:9.1.0016-1ubuntu7.15|
|vlc|3.0.20-3build6|
|vulkan-tools|1.3.275.0+dfsg1-1|
|vulkan-validationlayers|1.3.275.0-1|
|wamerican|2020.12.07-2|
|warpinator|2.0.4+zena|
|waybar|0.14.0-1ppa1|
|wayland-protocols|1.47-1ppa1|
|wayland-utils|1.2.0-1build1|
|wbritish|2020.12.07-2|
|webapp-manager|1.4.6|
|webp-pixbuf-loader|0.2.4-2build2|
|weston|13.0.0-4build3|
|wget|1.21.4-1ubuntu4.1|
|whiptail|0.52.24-2ubuntu2|
|winbind|2:4.19.5+dfsg-4ubuntu9.6|
|wine32|9.0~repack-4build3|
|wine64|9.0~repack-4build3|
|wine|9.0~repack-4build3|
|winetricks|20240105-2|
|wireless-regdb|2025.10.07-0ubuntu1~24.04.1|
|wireless-tools|30~pre9-16.1ubuntu2|
|wireplumber|0.4.17-1ubuntu4.1|
|wireshark|4.2.2-1.1build3|
|wl-clipboard|2.2.1-1build1|
|wlogout|1.1.1-3build2|
|wmctrl|1.07-7ubuntu3|
|wpasupplicant|2:2.10-21ubuntu0.4|
|x11-apps|7.7+11build3|
|x11-common|1:7.7+23ubuntu3|
|x11-session-utils|7.7+6build2|
|x11-utils|7.7+6build2|
|x11-xkb-utils|7.7+8build2|
|x11-xserver-utils|7.7+10build2|
|xapp-appimage-thumbnailer|1.2.9|
|xapp-epub-thumbnailer|1.2.9|
|xapp-jxl-thumbnailer|1.2.9|
|xapp-mp3-thumbnailer|1.2.9|
|xapp-ora-thumbnailer|1.2.9|
|xapp-raw-thumbnailer|1.2.9|
|xapps-common|3.2.2+zena|
|xapp-symbolic-icons|1.0.9|
|xapp-thumbnailers-common|1.2.9|
|xauth|1:1.1.2-1build1|
|xawtv-plugins|3.107-2build3|
|xbitmaps|1.1.1-2.2|
|xbrlapi|6.6-4ubuntu5|
|xclip|0.13-3|
|xcvt|0.1.2-1build1|
|xdg-dbus-proxy|0.1.5-1ubuntu0.2|
|xdg-desktop-portal|1.20.0+ds-2ubuntu1|
|xdg-desktop-portal-gtk|1.15.1-1build2|
|xdg-desktop-portal-hyprland|1.3.12-1ppa2|
|xdg-desktop-portal-wlr|0.7.1-1build2|
|xdg-desktop-portal-xapp|1.1.3+zena|
|xdg-user-dirs|0.18-1build1|
|xdg-user-dirs-gtk|0.11-1build2|
|xdg-utils|1.1.3-4.1ubuntu3|
|xdotool|1:3.20160805.1-5build1|
|xed|3.8.9+zena|
|xed-common|3.8.9+zena|
|xed-dbg|3.8.9+zena|
|xfonts-base|1:1.0.5+nmu1|
|xfonts-encodings|1:1.0.5-0ubuntu2|
|xfonts-scalable|1:1.0.3-1.3|
|xfonts-utils|1:7.7+6build3|
|xfsprogs|6.6.0-1ubuntu2.1|
|xinit|1.4.1-0ubuntu5|
|xinput|1.6.4-1build1|
|xkb-data|2.41-2ubuntu1.1|
|xml-core|0.19|
|xmlto|0.0.28-2.1|
|xorg|1:7.7+23ubuntu3|
|xorg-docs-core|1:1.7.1-1.2|
|xreader|4.6.5+wilma|
|xreader-common|4.6.5+wilma|
|xreader-dbg|4.6.5+wilma|
|xserver-common|2:21.1.12-1ubuntu1.5|
|xserver-xephyr|2:21.1.12-1ubuntu1.5|
|xserver-xorg|1:7.7+23ubuntu3|
|xserver-xorg-core|2:21.1.12-1ubuntu1.5|
|xserver-xorg-input-all|1:7.7+23ubuntu3|
|xserver-xorg-input-libinput|1.4.0-1ubuntu24.04.1|
|xserver-xorg-input-wacom|1:1.2.0-1ubuntu2|
|xserver-xorg-legacy|2:21.1.12-1ubuntu1.5|
|xserver-xorg-video-amdgpu|23.0.0-1ubuntu0.24.04.1|
|xserver-xorg-video-ati|1:22.0.0-1build1|
|xserver-xorg-video-fbdev|1:0.5.0-2build2|
|xserver-xorg-video-intel|2:2.99.917+git20210115-1build1|
|xserver-xorg-video-qxl|0.1.6-1build1|
|xserver-xorg-video-radeon|1:22.0.0-1build1|
|xserver-xorg-video-vesa|1:2.6.0-1ubuntu0.1|
|xserver-xorg-video-vmware|1:13.4.0-1build1|
|xterm|390-1ubuntu3|
|xvfb|2:21.1.12-1ubuntu1.5|
|xviewer|3.4.16+zena|
|xviewer-dbg|3.4.16+zena|
|xviewer-plugins|3.4.3+zena|
|xwayland|2:24.1.6-1ubuntu0.1~kisak~n|
|xxd|2:9.1.0016-1ubuntu7.15|
|xz-utils|5.6.1+really5.4.5-1ubuntu0.3|
|yad|0.40.0-1build3|
|yaru-theme-icon|24.04.2-0ubuntu1|
|ydotool|0.1.8-3build1|
|ydotoold|0.1.8-3build1|
|yelp|42.2-1ubuntu0.24.04.1|
|yelp-xsl|42.1-2ubuntu0.24.04.1|
|yq|3.1.0-3|
|yt-dlp|2024.04.09-1|
|zenity|3.44.2-1+wilma|
|zenity-common|3.44.2-1+wilma|
|zfsutils-linux|2.2.2-0ubuntu9.4|
|zfs-zed|2.2.2-0ubuntu9.4|
|zip|3.0-13ubuntu0.2|
|zlib1g|1:1.3.dfsg-3.1ubuntu2.1|
|zlib1g|1:1.3.dfsg-3.1ubuntu2.1|
|zoxide|0.9.3-1|
|zstd|1.5.5+dfsg2-2build1.1|
