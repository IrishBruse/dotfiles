const test = [
  ...
  {
    scope: ["source.qbe keyword.operator"],
    settings: {
      foreground: "#56B6C2",
    },
  },
  {
    scope: "constant.other.qbe",
    settings: {
      foreground: "#61AFEF",
    },
  },
  {
    scope: "entity.name.type.numeric.qbe",
    settings: {
      foreground: "#E5C07B",
    },
  },
  {
    scope: ["entity.name.label.qbe"],
    settings: {
      foreground: "#E06C75",
    },
  },
  {
    scope: "keyword.operator.expression.pattern",
    settings: {
      foreground: "#C678DD",
    },
  },
  ...
  {
    scope: ["constant.character.escape", "constant.character.escape.cs"],
    settings: {
      foreground: "#56B6C2",
    },
  },
  ...
  {
    scope: "keyword.operator.expression.new",
    settings: {
      foreground: "#E06C75",
    },
  },
  ...
  {
    scope: ["source.cs keyword.operator", "variable.language.this.cs"],
    settings: {
      foreground: "#C678DD",
    },
  },
  ...
  },
  {
    scope: [
      ...
      "support.type.property-name.hurl",
    ],
    settings: {
      foreground: "#E06C75",
    },
  ...
  {
    scope: [
      "meta.template.expression.js punctuation.definition.string",
      "meta.template.expression.jsx punctuation.definition.string",
      "meta.template.expression.ts punctuation.definition.string",
      "meta.template.expression.tsx punctuation.definition.string",
    ],
    settings: {
      foreground: "#98C379",
    },
  ...
  {
    scope: ["support.type.property-name.array.toml"],
    settings: {
      foreground: "#C678DD",
    },
  },
  {
    scope: ["meta.object-literal.key.tsx", "meta.object-literal.key.ts"],
    settings: {
      foreground: "#ABB2BF",
    },
  },
...
const un = [...new Set(asd)];

// Combine objects with identical settings into one with merged scopes
function combineScopes(arr) {
  const map = new Map();
  arr.forEach(({ scope, settings }) => {
  ...
  return Array.from(map.entries()).map(([settingsStr, scopes]) => ({
...
// Usage example:
const combined = combineScopes(test);

console.log(JSON.stringify(combined, null, 2));
// Helper to extract language from scope string
function getLangFromScope(scope) {
  const scopes = Array.isArray(scope) ? scope : [scope];
  for (const s of scopes) {
    // Match source.<lang> or file extension
    const m = s.match(
      /source\.(js|ts|jsx|tsx|python|css|cpp|c|go|json|php|ruby|elixir|rust|gfm|ini|java|xml|html)/
    );
    if (m) {
      const lang = m[1];
      if (["js", "ts", "jsx", "tsx"].includes(lang)) return "javascript";
      return lang;
    }
    // Match extension
    const ext = s.match(/\.([a-z]+)$/);
    if (ext) {
      if (["js", "ts", "jsx", "tsx"].includes(ext[1])) return "javascript";
      return ext[1];
    }
  }
  return "other";
...
// Split combined by language
function splitByLanguage(combinedArr) {
  const langMap = {};
  for (const obj of combinedArr) {
    const lang = getLangFromScope(obj.scope);
    if (!langMap[lang]) langMap[lang] = [];
    langMap[lang].push(obj);
  }
  return langMap;
}

const langSplit = splitByLanguage(combined);
console.log(JSON.stringify(langSplit, null, 2));
