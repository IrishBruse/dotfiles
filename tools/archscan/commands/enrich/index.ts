...
function computeInterfaceSurface(m: ModuleInfo): number {
  return (
    m.exports.types.length +
    m.exports.functions.length +
    m.exports.classes.length +
    m.exports.constants.length
  );
}
...
function assessDepth(
  m: ModuleInfo,
  depthRatio: number
): { assessment: "Deep" | "Shallow" | "Unclear"; reason: string } {
  ...
  if (
    depthRatio <= SHALLOW_DEPTH_RATIO_THRESHOLD &&
    m.fanIn <= LOW_FAN_IN &&
    m.exports.functions.length <= 2
  ) {
    if (m.loc < 30) {
      return {
        assessment: "Shallow",
...
function isPassThrough(m: ModuleInfo): boolean {
  ...
  if (m.loc < 20 && m.exports.functions.length <= 1 && m.fanIn === 0)
    return true;
  return false;
}
...
export async function runEnrich(args: string[]): Promise<void> {
  ...
  const inPath =
    args[1] ?? path.join(targetDir, ".context", "architecture-data.json");
  const outPath =
    args[2] ?? path.join(targetDir, ".context", "architecture-data.json");
...
function computeSummary(data: EnrichedProjectInfo) {
  ...
  const shallow = data.modules.filter(
    (m) => m.depthAssessment === "Shallow"
  ).length;
  const unclear = data.modules.filter(
    (m) => m.depthAssessment === "Unclear"
  ).length;
  const realSeams = data.modules.filter((m) => m.seamQuality === "Real").length;
  const hypotheticalSeams = data.modules.filter(
    (m) => m.seamQuality === "Hypothetical"
  ).length;
  const missingSeams = data.modules.filter(
    (m) => m.seamQuality === "Missing"
  ).length;
