...
const LINT_SKILL_ROOT_SUFFIXES = [
  [".agents", "skills"],
  [".cursor", "skills"],
] as const;
...
export function globalSkillRoots(): SkillRoot[] {
  const home = homedir();
  return SKILL_ROOT_SUFFIXES.map((suffix) => ({
    scope: "global",
    path: skillRootPath(home, suffix),
  }));
}

export function projectSkillRoots(startDir: string): SkillRoot[] {
  const roots: SkillRoot[] = [];
  const seen = new Set<string>();
  let dir = path.resolve(startDir);

  while (true) {
    for (const suffix of SKILL_ROOT_SUFFIXES) {
...
export function allSkillRoots(cwd: string): SkillRoot[] {
  return [...globalSkillRoots(), ...projectSkillRoots(cwd)];
}

export function defaultSkillRoots(): string[] {
  return globalSkillRoots().map((root) => root.path);
}

export function defaultLintRoots(): string[] {
  const home = homedir();
  return LINT_SKILL_ROOT_SUFFIXES.map((suffix) => skillRootPath(home, suffix));
}
