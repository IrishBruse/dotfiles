...
describe("formatDiagnostic", () => {
  it("uses eslint-style location, message, and @rule id", () => {
    ...
    assert.match(line, /@skill\/prose-semicolon\(fixable\)/);
  ...
  it("prints errors with an error severity label", () => {
    ...
    assert.match(line, /@skill\/skill-length/);
  });
  ...
  it("styles fixable suffix in green when color is enabled", () => {
    ...
    if (outputColorEnabled()) {
      assert.match(line, /\u001b\[2m@skill\/prose-semicolon\u001b\[0m\u001b\[32m\(fixable\)/);
    } else {
      assert.match(line, /@skill\/prose-semicolon\(fixable\)/);
    }
  });
...
describe("formatFileDiagnostics", () => {
  it("prints the path once with indented line:col diagnostics", () => {
    ...
    assert.match(output, /^\s+8:12\s+warning\s+Prefer a comma.*@skill\/prose-semicolon\(fixable\)/m);
    assert.match(output, /^\s+1:1\s+error  \s+SKILL\.md exceeds.*@skill\/skill-length$/m);
  ...
  it("shortens home paths to ~/", () => {
    ...
    assert.match(output, /^\s+3:1\s+warning\s+Prefer a comma.*@skill\/prose-semicolon/m);
  });
  ...
  it("aligns columns across diagnostics in a file", () => {
    ...
    const ruleStarts = detailLines.map((line) => line.indexOf("@skill/"));
    ...
    assert.match(detailLines[0] ?? "", /^\s+2:15\s{5}warning\s+Missing semicolon\s+@skill\/prose-semicolon\(fixable\)$/);
    assert.match(detailLines[1] ?? "", /^\s+6:7\s+error  \s+'result' is assigned.*@skill\/negation-steering$/);
