...
`skill lint` checks agent skill markdown the way ESLint checks JavaScript:
each rule has a stable code, a severity, and (sometimes) an auto-fix.

Rules live under `tools/skill/rules/`, grouped by what they care about.
...
For authoring principles and CLI usage (`skill lint`, `skill ls`), see `writing-great-skills`
...
```bash
cd tools
node --test skill/rules/**/*.test.ts skill/commands/lint.test.ts
```
