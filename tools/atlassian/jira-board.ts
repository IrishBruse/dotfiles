...
function runInit(userPath: string | undefined): number {
  const skillDir = userPath
    ? path.resolve(userPath)
    : path.join(TOOL_DIR, "..", "..", "home", ".agents", "skills", "jira-tickets");

  const refsDir = path.join(skillDir, "references");
  const skillMdPath = path.join(skillDir, "SKILL.md");

  if (fs.existsSync(skillMdPath)) {
    console.error(`jira-board init: skill file already exists at ${skillMdPath}`);
    return 1;
  }
...
function printHelp(): void {
  ...
  jira-board init [path]  create empty skill directory structure
