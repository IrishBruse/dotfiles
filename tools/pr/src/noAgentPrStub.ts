 ...
 */
export function seedNoAgentPrCreateStub(agentOutputDir: string): void {
 ...
 */
export function seedNoAgentPrReviewStub(agentOutputDir: string): void {
  const title = isPrCliWork()
...
/**
 * `pr update --no-agent`: prefetched GitHub state is in the prompt as **`CURRENT.md`**; write a
 * placeholder **`PR.md`** so {@link readAgentPrMarkdown} and the editor step can run.
 */
export function seedNoAgentPrUpdateStub(agentOutputDir: string): void {
  const title = isPrCliWork()
