 ...
 * scripts/<name>.sh (adds .sh if missing) with stdin = inner text, replaces inner
 * with stdout. cwd = scripts/. Always allows the Read.
 */
...
function underScripts(scriptsDir: string, rel: string): string | null {
...
function main(): number {
  ...
  const runScript = (markerName: string, stdin: string | null | undefined) => {
    ...
    const script = underScripts(scriptsDir, file);
