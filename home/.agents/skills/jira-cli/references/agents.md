...
- `jira info` -- workspace context (CONFIG, board cache, local ticket count, cached issue types)
- `jira doctor --json` -- verify acli, auth, CONFIG, and cache before retrying failures
- `jira sync` -- refresh `~/.agents/skills/jira-board/` (generated at runtime, not in dotfiles)
...
Allowed subcommands: `info`, `show`, `search`, `projects`, `types`.
...
| `--session` + `--restore` | Local `jira/` markdown + `~/.agents/skills/jira-board/` cache |
