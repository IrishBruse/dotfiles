...
10. Promote only on explicit confirmation. Use `sdd-plan` Jira Promotion for active SDD drafts. Otherwise call `createJiraIssue`.
...
For NOVACORE, Feature Team is `customfield_10354`, multi-select, pass option ids. Resolve in order: parent Initiative, sibling Epic, ask once.
...
| `description` | Context + Expectations + High-level Scenarios as markdown |
| `parent` | Verified Initiative key |
| `contentFormat` | `markdown` |
| `additional_fields` | Feature Team when known, Capitalizable for NOVACORE, optional priority, labels, components |
...
| Draft confirmation | Before `createJiraIssue` | Save local draft and wait for explicit yes or file edits |
...
- Do not set an individual assignee.
