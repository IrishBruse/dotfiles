...
9. Promote only on explicit confirmation. Use the appropriate promotion flow for the draft context, or call `createJiraIssue` when no promotion flow applies.
...
| `issueTypeName` | `Task`, or `Sub-task` when the user confirmed a Sub-task |
| `summary` | Draft summary |
| `description` | Template body as markdown |
| `parent` | Epic key when available for Tasks; required parent issue key for Sub-tasks |
...
- Do not create without a local draft and explicit publish confirmation.
