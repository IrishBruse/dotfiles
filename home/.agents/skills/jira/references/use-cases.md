...
This route is for Jira-first Epic discovery. For local-first SDD work, persist agreed use cases using workspace artifact conventions and record the source Epic in the active change context. Posting to Jira is optional and requires explicit confirmation.
...
For local persistence, write agreed use cases using workspace artifact conventions. If no active change context exists, offer to route through `sdd-init`.
...
Post using `addCommentToJiraIssue` only after explicit confirmation.
